...
private val NativeClass.capName: String
	get() = if ( templateName.startsWith(prefix) ) {
		if ( prefix == "ALC" )
			"OpenALC${templateName.substring(3)}"
		else
			templateName
	} else {
	...
	private fun PrintWriter.generateCapabilitiesImpl() {
		...
		val classes = super.getClasses(object : Comparator<NativeClass> {
			...
			public override fun compare(o1: NativeClass, o2: NativeClass): Int {
				val isALC1 = o1.capName.startsWith("ALC")
				val isALC2 = o2.capName.startsWith("ALC")
		...
		for ( extension in classes ) {
			if ( extension.hasNativeFunctions ) {
				println("\t\t${extension.capName} = (__${extension.className} = ${extension.className}.create(ext, provider)) != null;")
			} else
...
private fun String.addCapabilities(templateName: String, nativeSubPath: String, prefix: String, postfix: String, init: NativeClass.() -> Unit): NativeClass {
	val extension = nativeClass("org.lwjgl.openal", templateName, nativeSubPath, prefix, postfix, FunctionProviderALC, init)
	FunctionProviderALC.addCapabilities(extension)
	return extension
}

public fun String.nativeClassALC(templateName: String, postfix: String = "", init: NativeClass.() -> Unit): NativeClass {
	return addCapabilities(templateName, "", "ALC", postfix, init)
}