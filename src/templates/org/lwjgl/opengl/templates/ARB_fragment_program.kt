...
val ARB_fragment_program = "ARBFragmentProgram".nativeClassGL("ARB_fragment_program", postfix = ARB) {
	documentation =
		...
		capabilities (${ARB_texture_env_add.link}, ${ARB_texture_env_combine.link}, ${ARB_texture_env_dot3.link}, ${ARB_texture_env_crossbar.link}),
		per-fragment depth comparisons (${ARB_depth_texture.link}, ${ARB_shadow.link}, ${ARB_shadow_ambient.link}, ${EXT_shadow_funcs.link}),
		per-fragment lighting (${registryLinkTo("EXT", "fragment_lighting")}, ${registryLinkTo("EXT", "light_texture")}), and environment mapped bump
		mapping (${registryLinkTo("ATI", "envmap_bumpmap")}).
