...
fun Xlib() = "Xlib".nativeClass(LINUX_PACKAGE) {
	...
	int.func(
		...
		changed (see ${"XSetCloseDownMode()".link}). Therefore, these windows, resource IDs, and other resources should never be referenced again or an error will
	...
	Bool.func(
		"XQueryExtension",
		"""
		Determines if the named extension is present. If the extension is not present, {@code XQueryExtension()} returns ${"False".link}; otherwise, it returns
		${"True".link}. If the extension is present, {@code XQueryExtension()} returns the major opcode for the extension to {@code major_opcode_return};
	...
	int.func(
		...
		${"XPending()".link}, ${"XNextEvent()".link}, and ${"XWindowEvent()".link}. Events generated by the server may be enqueued into the library's event queue.
		""",
	...
	int.func(
		...
		to ${"XEventsQueued()".link} with the mode {@code QueuedAfterFlush} specified.
		""",
	...
	int.func(
		...
		"Depending on the {@code valuemask}, uses the window attributes in the ${"XSetWindowAttributes".link} structure to change the specified window attributes.",
		...
		XSetWindowAttributes_p.IN(
			"attributes",
			"the ${"XSetWindowAttributes".link} structure from which the values (as specified by the value mask) are to be taken."
		)
	)
	...
	int.func(
		"XDefineCursor",
		"If a cursor is set, it will be used when the pointer is in the window. If the cursor is $None, it is equivalent to ${"XUndefineCursor()".link}.",
	...
	int.func(
		"XUndefineCursor",
		"""
		Undoes the effect of a previous ${"XDefineCursor()".link} for this window. When the pointer is in the window, the parent's cursor will now be used. On the
	...
	Status.func(
		...
		${ul(
			...
			event is sent.
			""",
			"If propagate is ${"False".link}, the event is sent to every client selecting on destination any of the event types in the {@code event_mask} argument.",
			"""
	...
	int.func(
		"XMapRaised",
		"""
		Essentially is similar to ${"XMapWindow()".link} in that it maps the window and all of its subwindows that have had map requests. However, it also raises
	...
	int.func(
		...
		but the last-change time is not affected. The selection atom is uninterpreted by the X server. ${"XGetSelectionOwner()".link} returns the owner window,
	...
	Bool.func(
		...
		match, {@code XCheckTypedEvent()} removes that event, copies it into the specified {@link XEvent} structure, and returns ${"True".link}. The other events
		in the queue are not discarded. If the event is not available, {@code XCheckTypedEvent()} returns ${"False".link}, and the output buffer will have been
		flushed.
		""",
