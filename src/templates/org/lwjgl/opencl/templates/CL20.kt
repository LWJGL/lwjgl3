...
val INVALID_PIPE_SIZE = "CL20".linkCL("INVALID_PIPE_SIZE")
...
fun CL20() = "CL20".nativeClassCL("CL20") {
	...
	(Construct("context", "device") _ cl_command_queue).func(
		...
		cl_device_id.IN(
			"device",
			"""
			a device associated with context. It can either be in the list of devices specified when context is created using ${"CL10".link("CreateContext()")} or have the same
			device type as device type specified when context is created using ${"CL10".link("CreateContextFromType()")}.
			"""
		),
	...
	(Construct("context") _ cl_mem).func(
		...
		cl_mem_flags.IN(
			...
			how it will be used. If value specified for flags is 0, the default is used which is ${"CL10".link("MEM_READ_WRITE")}.
			"""
		),
		...
		${ul(
			...
			"""
			$INVALID_PIPE_SIZE if {@code pipe_packet_size} is {@code 0} or the {@code pipe_packet_size} exceeds ${"CL20".link("DEVICE_PIPE_MAX_PACKET_SIZE")}
			value for all devices in {@code context} or if {@code pipe_max_packets} is {@code 0}.
			""",
			"${"CL10".link("MEM_OBJECT_ALLOCATION_FAILURE")} if there is a failure to allocate memory for the pipe object.",
			OORE,
            OOHME
		)}
	    """
	...
	val SVMAlloc = void_p.func(
		...
		If ${"MEM_SVM_FINE_GRAIN_BUFFER".link} is not specified, the buffer can be created as a coarse grained SVM allocation. Similarly, if
		${"MEM_SVM_ATOMICS".link} is not specified, the buffer can be created without support for the OpenCL 2.0 SVM atomic operations.
        ...
        Although SVM is generally not supported for image objects, ${"CL12".link("CreateImage()")} may create an image from a buffer (a 1D image from a buffer
        ...
        If ${"CL10".link("CreateBuffer()")} is called with a pointer returned by {@code SVMAlloc} as its {@code host_ptr} argument, and
        $("CL10".link("MEM_USE_HOST_PTR")} is set in its {@code flags} argument, {@code CreateBuffer} will succeed and return a valid non-zero buffer object as
		...
		cl_svm_mem_flags.IN(
			...
			"CL10#MEM_READ_WRITE CL10#MEM_WRITE_ONLY CL10#MEM_READ_ONLY CL10#MEM_SVM_FINE_GRAIN_BUFFER CL10#MEM_SVM_ATOMICS"
		),
		...
		${ul(
			...
			"{@code flags} does not contain ${"MEM_SVM_FINE_GRAIN_BUFFER".link} but does contain ${"MEM_SVM_ATOMICS".link}.",
			"Values specified in {@code flags} do not follow rules for supported values.",
			"${"MEM_SVM_FINE_GRAIN_BUFFER".link} or ${"MEM_SVM_ATOMICS".link} is specified in flags and these are not supported by at least one device in context.",
			"The values specified in {@code flags} are not valid.",
			"{@code size} is {@code 0} or &gt; ${"DEVICE_MAX_MEM_ALLOC_SIZE".link} value for any device in context.",
	...
	void.func(
		...
		freeing {@code svm_pointer}. This can be done by enqueuing a blocking operation such as ${"CL10".link("Finish()")}, ${"CL10".link("WaitForEvents()")},
		${"CL10".link("EnqueueReadBuffer()")} or by registering a callback with the events associated with enqueued commands and when the last enqueued command
        ...
        ${"CL10".link("CreateBuffer()")} with {@code svm_pointer}, the buffer object must first be released before the {@code svm_pointer} is freed.
		""",
	...
	cl_int.func(
        ...
        ${ul(
			...
			"${"CL10".link("MEM_COPY_OVERLAP")} if the values specified for {@code dst_ptr}, {@code src_ptr} and {@code size} result in an overlapping copy.",
			OORE,
			OOHME
		)}
        """
	...
	cl_int.func(
		...
		const _ void_p.IN(
			...
			by all API calls that enqueue kernel (${"CL10".link("EnqueueNDRangeKernel()")}) until the argument value is changed by a call to
	...
	cl_int.func(
		...
		{@code SetKernelExecInfo} with ${"KERNEL_EXEC_INFO_SVM_PTRS".link}. For example, if SVM buffer A contains a pointer to another SVM buffer B, and the
        ...
        support system SVM pointers, ${"CL10".link("EnqueueNDRangeKernel()")} will return a $INVALID_OPERATION error. If none of the devices in the context
