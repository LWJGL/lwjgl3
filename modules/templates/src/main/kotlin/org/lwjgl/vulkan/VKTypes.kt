...
￿    VkSystemAllocationScope                     allocationScope);</code></pre>
		...
		If {@code pfnAllocation} returns {@code NULL}, and if the implementation is unable to continue correct processing of the current command without the requested allocation, it <b>must</b> treat this as a run-time error, and generate #ERROR_OUT_OF_HOST_MEMORY at the appropriate time for the command in which the condition was detected, as described in <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#fundamentals-errorcodes"> Return Codes</a>.
...
￿    VkSystemAllocationScope                     allocationScope);</code></pre>
		...
		{@code pfnReallocation} <b>must</b> follow the same <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#vkAllocationFunction_return_rules"> rules for return values as tname:PFN_vkAllocationFunction</a>.
...
val VkInstanceCreateInfo = struct(VULKAN_PACKAGE, "VkInstanceCreateInfo") {
	...
	const..charUTF8_p_const_p.member("ppEnabledLayerNames", "a pointer to an array of {@code enabledLayerCount} null-terminated UTF-8 strings containing the names of layers to enable for the created instance. See the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#extended-functionality-layers\">Layers</a> section for further details.")
...
val VkPhysicalDeviceFeatures = struct(VULKAN_PACKAGE, "VkPhysicalDeviceFeatures") {
	documentation =
		...
		If a SPIR-V {@code OpLoad} instruction loads a structure and the tail end of the structure is out of bounds, then all members of the structure are considered out of bounds even if the members at the end are not statically used.
			...
			<li>{@code fullDrawIndexUint32} indicates the full 32-bit range of indices is supported for indexed draw calls when using a {@code VkIndexType} of #INDEX_TYPE_UINT32. {@code maxDrawIndexedIndexValue} is the maximum index value that <b>may</b> be used (aside from the primitive restart index, which is always 2<sup>32</sup>-1 when the {@code VkIndexType} is #INDEX_TYPE_UINT32). If this feature is supported, {@code maxDrawIndexedIndexValue} <b>must</b> be 2<sup>32</sup>-1; otherwise it <b>must</b> be no smaller than 2<sup>24</sup>-1. See <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-limits-maxDrawIndexedIndexValue">maxDrawIndexedIndexValue</a>.</li>
			...
			<li>{@code multiDrawIndirect} indicates whether multiple draw indirect is supported. If this feature is not enabled, the {@code drawCount} parameter to the #CmdDrawIndirect() and #CmdDrawIndexedIndirect() commands <b>must</b> be 0 or 1. The {@code maxDrawIndirectCount} member of the ##VkPhysicalDeviceLimits structure <b>must</b> also be 1 if this feature is not supported. See <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-limits-maxDrawIndirectCount">maxDrawIndirectCount</a>.</li>
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If any member of this structure is #FALSE, as returned by #GetPhysicalDeviceFeatures(), then it <b>must</b> be #FALSE when passed as part of the ##VkDeviceCreateInfo struct when creating a device</li>
		</ul>
...
￿    VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_IMG = 0x00002000,
		...
		The following bits <b>may</b> be set in {@code linearTilingFeatures} and {@code optimalTilingFeatures}, indicating they are supported by images or image views created with the queried #GetPhysicalDeviceFormatProperties(){@code ::format}:

		<dl>
			<dt>#FORMAT_FEATURE_SAMPLED_IMAGE_BIT</dt>
			...
			<dt>#FORMAT_FEATURE_STORAGE_IMAGE_BIT</dt>
			...
			<dt>#FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT</dt>
			<dd>{@code VkImageView} <b>can</b> be used as a framebuffer depth/stencil attachment and as an input attachment.</dd>

			<dt>#FORMAT_FEATURE_BLIT_SRC_BIT</dt>
			<dd>{@code VkImage} <b>can</b> be used as {@code srcImage} for the #CmdBlitImage() command.</dd>

			<dt>#FORMAT_FEATURE_BLIT_DST_BIT</dt>
		...
		The following features <b>may</b> appear in {@code bufferFeatures}, indicating they are supported by buffers or buffer views created with the queried #GetPhysicalDeviceFormatProperties(){@code ::format}:

		<dl>
			<dt>#FORMAT_FEATURE_UNIFORM_TEXEL_BUFFER_BIT</dt>
			<dd>Format <b>can</b> be used to create a {@code VkBufferView} that <b>can</b> be bound to a #DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER descriptor.</dd>

			<dt>#FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_BIT</dt>
			<dd>Format <b>can</b> be used to create a {@code VkBufferView} that <b>can</b> be bound to a #DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER descriptor.</dd>

			<dt>#FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_ATOMIC_BIT</dt>
			<dd>Atomic operations are supported on #DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER with this format.</dd>

			<dt>#FORMAT_FEATURE_VERTEX_BUFFER_BIT</dt>
			<dd>Format <b>can</b> be used as a vertex attribute format (##VkVertexInputAttributeDescription{@code ::format}).</dd>
		</dl>
	...
	VkFormatFeatureFlags.member("optimalTilingFeatures", "describes the features supported by #IMAGE_TILING_OPTIMAL.")
	VkFormatFeatureFlags.member("bufferFeatures", "describes the features supported by buffers.")
}
...
val VkImageFormatProperties = struct(VULKAN_PACKAGE, "VkImageFormatProperties", mutable = false) {
	...
	VkSampleCountFlags.member("sampleCounts", "a bitmask of {@code VkSampleCountFlagBits} specifying all the supported sample counts for this image as described <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#features-supported-sample-counts\">below</a>.")
...
￿    VK_SAMPLE_COUNT_64_BIT = 0x00000040,
			...
			The sample count limits defined above represent the minimum supported sample counts for each image type. Individual images <b>may</b> support additional sample counts, which are queried using #GetPhysicalDeviceImageFormatProperties() as described in <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-supported-sample-counts"> Supported Sample Counts</a>.</dd>
		</dl>
	...
	uint32_t.member("maxImageDimension2D", "the maximum dimension ({@code width} or {@code height}) of an image created with an {@code imageType} of #IMAGE_TYPE_2D and without #IMAGE_CREATE_CUBE_COMPATIBLE_BIT set in {@code flags}.")
	uint32_t.member("maxImageDimension3D", "the maximum dimension ({@code width}, {@code height}, or {@code depth}) of an image created with an {@code imageType} of #IMAGE_TYPE_3D.")
	uint32_t.member("maxImageDimensionCube", "the maximum dimension ({@code width} or {@code height}) of an image created with an {@code imageType} of #IMAGE_TYPE_2D and with #IMAGE_CREATE_CUBE_COMPATIBLE_BIT set in {@code flags}.")
	...
	uint32_t.member("maxPushConstantsSize", "the maximum size, in bytes, of the pool of push constant memory. For each of the push constant ranges indicated by the {@code pPushConstantRanges} member of the ##VkPipelineLayoutCreateInfo structure, {@code offset} + {@code size} <b>must</b> be less than or equal to this limit.")
	...
	uint32_t.member("maxBoundDescriptorSets", "the maximum number of descriptor sets that <b>can</b> be simultaneously used by a pipeline. All {@code DescriptorSet} decorations in shader modules <b>must</b> have a value less than {@code maxBoundDescriptorSets}. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-sets\">the “Descriptor Sets” section</a>.")
	uint32_t.member("maxPerStageDescriptorSamplers", "the maximum number of samplers that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_SAMPLER or #DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER count against this limit. A descriptor is accessible to a shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-sampler\">the “Sampler” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-combinedimagesampler\">the “Combined Image Sampler” section</a>.")
	uint32_t.member("maxPerStageDescriptorUniformBuffers", "the maximum number of uniform buffers that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_UNIFORM_BUFFER or #DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC count against this limit. A descriptor is accessible to a shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformbuffer\">the “Uniform Buffer” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformbufferdynamic\">the “Dynamic Uniform Buffer” section</a>.")
	uint32_t.member("maxPerStageDescriptorStorageBuffers", "the maximum number of storage buffers that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_STORAGE_BUFFER or #DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC count against this limit. A descriptor is accessible to a pipeline shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagebuffer\">the “Storage Buffer” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagebufferdynamic\">the “Dynamic Storage Buffer” section</a>.")
	uint32_t.member("maxPerStageDescriptorSampledImages", "the maximum number of sampled images that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, #DESCRIPTOR_TYPE_SAMPLED_IMAGE, or #DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER count against this limit. A descriptor is accessible to a pipeline shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-combinedimagesampler\">the “Combined Image Sampler” section</a>, <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-sampledimage\">the “Sampled Image” section</a>, and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformtexelbuffer\">the “Uniform Texel Buffer” section</a>.")
	uint32_t.member("maxPerStageDescriptorStorageImages", "the maximum number of storage images that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_STORAGE_IMAGE, or #DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER count against this limit. A descriptor is accessible to a pipeline shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storageimage\">the “Storage Image” section</a>, and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagetexelbuffer\">the “Storage Texel Buffer” section</a>.")
	uint32_t.member("maxPerStageDescriptorInputAttachments", "the maximum number of input attachments that <b>can</b> be accessible to a single shader stage in a pipeline layout. Descriptors with a type of #DESCRIPTOR_TYPE_INPUT_ATTACHMENT count against this limit. A descriptor is accessible to a pipeline shader stage when the {@code stageFlags} member of the ##VkDescriptorSetLayoutBinding structure has the bit for that shader stage set. These are only supported for the fragment stage. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-inputattachment\">the “Input Attachment” section</a>.")
	...
	uint32_t.member("maxDescriptorSetSamplers", "the maximum number of samplers that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_SAMPLER or #DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-sampler\">the “Sampler” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-combinedimagesampler\">the “Combined Image Sampler” section</a>.")
	uint32_t.member("maxDescriptorSetUniformBuffers", "the maximum number of uniform buffers that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_UNIFORM_BUFFER or #DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformbuffer\">the “Uniform Buffer” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformbufferdynamic\">the “Dynamic Uniform Buffer” section</a>.")
	uint32_t.member("maxDescriptorSetUniformBuffersDynamic", "the maximum number of dynamic uniform buffers that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformbufferdynamic\">the “Dynamic Uniform Buffer” section</a>.")
	uint32_t.member("maxDescriptorSetStorageBuffers", "the maximum number of storage buffers that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_STORAGE_BUFFER or #DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagebuffer\">the “Storage Buffer” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagebufferdynamic\">the “Dynamic Storage Buffer” section</a>.")
	uint32_t.member("maxDescriptorSetStorageBuffersDynamic", "the maximum number of dynamic storage buffers that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-storagebufferdynamic\">the “Dynamic Storage Buffer” section</a>.")
	uint32_t.member("maxDescriptorSetSampledImages", "the maximum number of sampled images that <b>can</b> be included in descriptor bindings in a pipeline layout across all pipeline shader stages and descriptor set numbers. Descriptors with a type of #DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, #DESCRIPTOR_TYPE_SAMPLED_IMAGE, or #DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER count against this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-combinedimagesampler\">the “Combined Image Sampler” section</a>, <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-sampledimage\">the “Sampled Image” section</a>, and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#descriptorsets-uniformtexelbuffer\">the “Uniform Texel Buffer” section</a>.")
	...
	uint32_t.member("maxVertexInputAttributes", "the maximum number of vertex input attributes that <b>can</b> be specified for a graphics pipeline. These are described in the array of ##VkVertexInputAttributeDescription structures that are provided at graphics pipeline creation time via the {@code pVertexAttributeDescriptions} member of the ##VkPipelineVertexInputStateCreateInfo structure. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fxvertex-attrib\">the “Vertex Attributes” section</a> and <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fxvertex-input\">the “Vertex Input Description” section</a>.")
	uint32_t.member("maxVertexInputBindings", "the maximum number of vertex buffers that <b>can</b> be specified for providing vertex attributes to a graphics pipeline. These are described in the array of ##VkVertexInputBindingDescription structures that are provided at graphics pipeline creation time via the {@code pVertexBindingDescriptions} member of the ##VkPipelineVertexInputStateCreateInfo structure. The {@code binding} member of ##VkVertexInputBindingDescription <b>must</b> be less than this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fxvertex-input\">the “Vertex Input Description” section</a>.")
	uint32_t.member("maxVertexInputAttributeOffset", "the maximum vertex input attribute offset that <b>can</b> be added to the vertex input binding stride. The {@code offset} member of the ##VkVertexInputAttributeDescription structure <b>must</b> be less than or equal to this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fxvertex-input\">the “Vertex Input Description” section</a>.")
	uint32_t.member("maxVertexInputBindingStride", "the maximum vertex input binding stride that <b>can</b> be specified in a vertex input binding. The {@code stride} member of the ##VkVertexInputBindingDescription structure <b>must</b> be less than or equal to this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fxvertex-input\">the “Vertex Input Description” section</a>.")
	uint32_t.member("maxVertexOutputComponents", "the maximum number of components of output variables which <b>can</b> be output by a vertex shader. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#shaders-vertex\">the “Vertex Shaders” section</a>.")
	uint32_t.member("maxTessellationGenerationLevel", "the maximum tessellation generation level supported by the fixed-function tessellation primitive generator. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#tessellation\">the “Tessellation” chapter</a>.")
	uint32_t.member("maxTessellationPatchSize", "the maximum patch size, in vertices, of patches that <b>can</b> be processed by the tessellation control shader and tessellation primitive generator. The {@code patchControlPoints} member of the ##VkPipelineTessellationStateCreateInfo structure specified at pipeline creation time and the value provided in the {@code OutputVertices} execution mode of shader modules <b>must</b> be less than or equal to this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#tessellation\">the “Tessellation” chapter</a>.")
	...
	uint32_t.member("maxGeometryShaderInvocations", "the maximum invocation count supported for instanced geometry shaders. The value provided in the {@code Invocations} execution mode of shader modules <b>must</b> be less than or equal to this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#geometry\">the “Geometry Shading” chapter</a>.")
	...
	uint32_t.array("maxComputeWorkGroupCount", "the maximum number of local workgroups that <b>can</b> be dispatched by a single dispatch command. These three values represent the maximum number of local workgroups for the X, Y, and Z dimensions, respectively. The {@code x}, {@code y}, and {@code z} parameters to the #CmdDispatch() command, or members of the ##VkDispatchIndirectCommand structure <b>must</b> be less than or equal to the corresponding limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#dispatch\">the “Dispatching Commands” chapter</a>.", size = 3)
	...
	uint32_t.member("subPixelPrecisionBits", "the number of bits of subpixel precision in framebuffer coordinates <code>x<sub>f</sub></code> and <code>y<sub>f</sub></code>. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#primsrast\">the “Rasterization” chapter</a>.")
...
<div style="margin-left: 26px; border-left: 1px solid gray; padding-left: 14px;"><h5>Note</h5>
	...
	uint32_t.member("maxDrawIndexedIndexValue", "the maximum index value that <b>can</b> be used for indexed draw calls when using 32-bit indices. This excludes the primitive restart index value of 0xFFFFFFFF. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#features-features-fullDrawIndexUint32\">fullDrawIndexUint32</a>.")
	uint32_t.member("maxDrawIndirectCount", "the maximum draw count that is supported for indirect draw calls. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#features-features-multiDrawIndirect\">multiDrawIndirect</a>.")
	float.member("maxSamplerLodBias", "the maximum absolute sampler level of detail bias. The sum of the {@code mipLodBias} member of the ##VkSamplerCreateInfo structure and the {@code Bias} operand of image sampling operations in shader modules (or 0 if no {@code Bias} operand is provided to an image sampling operation) are clamped to the range <code>[-maxSamplerLodBias,+maxSamplerLodBias]</code>. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#samplers-mipLodBias\">samplers-mipLodBias</a>.")
	float.member("maxSamplerAnisotropy", "the maximum degree of sampler anisotropy. The maximum degree of anisotropic filtering used for an image sampling operation is the minimum of the {@code maxAnisotropy} member of the ##VkSamplerCreateInfo structure and this limit. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#samplers-maxAnisotropy\">samplers-maxAnisotropy</a>.")
	...
	uint32_t.array("maxViewportDimensions", "are the maximum viewport dimensions in the X (width) and Y (height) dimensions, respectively. The maximum viewport dimensions <b>must</b> be greater than or equal to the largest image which <b>can</b> be created and used as a framebuffer attachment. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#vertexpostproc-viewport\">Controlling the Viewport</a>.", size = 2)
	float.array("viewportBoundsRange", """the <code>[minimum, maximum]</code> range that the corners of a viewport <b>must</b> be contained in. This range <b>must</b> be at least
<code>[-2 {times} size, 2 {times} size - 1]</code>, where <code>size = max(maxViewportDimensions[0], maxViewportDimensions[1])</code> See <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#vertexpostproc-viewport">Controlling the Viewport</a>.
	...
	VkSampleCountFlags.member("framebufferColorSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the color sample counts that are supported for all framebuffer color attachments.")
	VkSampleCountFlags.member("framebufferDepthSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the supported depth sample counts for all framebuffer depth/stencil attachments, when the format includes a depth component.")
	VkSampleCountFlags.member("framebufferStencilSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the supported stencil sample counts for all framebuffer depth/stencil attachments, when the format includes a stencil component.")
	VkSampleCountFlags.member("framebufferNoAttachmentsSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the supported sample counts for a framebuffer with no attachments.")
	...
	VkSampleCountFlags.member("sampledImageColorSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the sample counts supported for all 2D images created with #IMAGE_TILING_OPTIMAL, {@code usage} containing #IMAGE_USAGE_SAMPLED_BIT, and a non-integer color format.")
	VkSampleCountFlags.member("sampledImageIntegerSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the sample counts supported for all 2D images created with #IMAGE_TILING_OPTIMAL, {@code usage} containing #IMAGE_USAGE_SAMPLED_BIT, and an integer color format.")
	VkSampleCountFlags.member("sampledImageDepthSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the sample counts supported for all 2D images created with #IMAGE_TILING_OPTIMAL, {@code usage} containing #IMAGE_USAGE_SAMPLED_BIT, and a depth format.")
	VkSampleCountFlags.member("sampledImageStencilSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the sample supported for all 2D images created with #IMAGE_TILING_OPTIMAL, {@code usage} containing #IMAGE_USAGE_SAMPLED_BIT, and a stencil format.")
	VkSampleCountFlags.member("storageImageSampleCounts", "a bitmask<sup>1</sup> of {@code VkSampleCountFlagBits} bits indicating the sample counts supported for all 2D images created with #IMAGE_TILING_OPTIMAL, and {@code usage} containing #IMAGE_USAGE_STORAGE_BIT.")
	...
	VkBool32.member("timestampComputeAndGraphics", "indicates support for timestamps on all graphics and compute queues. If this limit is set to #TRUE, all queues that advertise the #QUEUE_GRAPHICS_BIT or #QUEUE_COMPUTE_BIT in the ##VkQueueFamilyProperties{@code ::queueFlags} support ##VkQueueFamilyProperties{@code ::timestampValidBits} of at least 36. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#queries-timestamps\">Timestamp Queries</a>.")
	float.member("timestampPeriod", "the number of nanoseconds required: for a timestamp query to be incremented by 1. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#queries-timestamps\">Timestamp Queries</a>.")
	...
	VkBool32.member("strictLines", "indicates whether lines are rasterized according to the preferred method of rasterization. If set to #FALSE, lines <b>may</b> be rasterized under a relaxed set of rules. If set to #TRUE, lines are rasterized as per the strict definition. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#primsrast-lines-basic\">Basic Line Segment Rasterization</a>.")
	VkBool32.member("standardSampleLocations", "indicates whether rasterization uses the standard sample locations as documented in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#primsrast-multisampling\">Multisampling</a>. If set to #TRUE, the implementation uses the documented sample locations. If set to #FALSE, the implementation <b>may</b> use different sample locations.")
	VkDeviceSize.member("optimalBufferCopyOffsetAlignment", "the optimal buffer offset alignment in bytes for #CmdCopyBufferToImage() and #CmdCopyImageToBuffer(). The per texel alignment requirements are still enforced, this is just an additional alignment recommendation for optimal performance and power.")
	VkDeviceSize.member("optimalBufferCopyRowPitchAlignment", "the optimal buffer row pitch alignment in bytes for #CmdCopyBufferToImage() and #CmdCopyImageToBuffer(). Row pitch is the number of bytes between texels with the same X coordinate in adjacent rows (Y coordinates differ by one). The per texel alignment requirements are still enforced, this is just an additional alignment recommendation for optimal performance and power.")
	VkDeviceSize.member("nonCoherentAtomSize", "the size and alignment in bytes that bounds concurrent access to <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#memory-device-hostaccess\">host-mapped device memory</a>.")
}
...
val VkPhysicalDeviceSparseProperties = struct(VULKAN_PACKAGE, "VkPhysicalDeviceSparseProperties", mutable = false) {
	...
	VkBool32.member("residencyStandard2DBlockShape", "#TRUE if the physical device will access all single-sample 2D sparse resources using the standard sparse image block shapes (based on image format), as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#sparsememory-sparseblockshapessingle\">Standard Sparse Image Block Shapes (Single Sample)</a> table. If this property is not supported the value returned in the {@code imageGranularity} member of the ##VkSparseImageFormatProperties structure for single-sample 2D images is not required: to match the standard sparse image block dimensions listed in the table.")
	VkBool32.member("residencyStandard2DMultisampleBlockShape", "#TRUE if the physical device will access all multisample 2D sparse resources using the standard sparse image block shapes (based on image format), as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#sparsememory-sparseblockshapesmsaa\">Standard Sparse Image Block Shapes (MSAA)</a> table. If this property is not supported, the value returned in the {@code imageGranularity} member of the ##VkSparseImageFormatProperties structure for multisample 2D images is not required: to match the standard sparse image block dimensions listed in the table.")
	VkBool32.member("residencyStandard3DBlockShape", "#TRUE if the physical device will access all 3D sparse resources using the standard sparse image block shapes (based on image format), as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#sparsememory-sparseblockshapessingle\">Standard Sparse Image Block Shapes (Single Sample)</a> table. If this property is not supported, the value returned in the {@code imageGranularity} member of the ##VkSparseImageFormatProperties structure for 3D images is not required: to match the standard sparse image block dimensions listed in the table.")
...
val VkPhysicalDeviceProperties = struct(VULKAN_PACKAGE, "VkPhysicalDeviceProperties", mutable = false) {
	...
	VkPhysicalDeviceLimits.member("limits", "the ##VkPhysicalDeviceLimits structure which specifies device-specific limits of the physical device. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#features-limits\">Limits</a> for details.")
	VkPhysicalDeviceSparseProperties.member("sparseProperties", "the ##VkPhysicalDeviceSparseProperties structure which specifies various sparse related properties of the physical device. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#sparsememory-physicalprops\">Sparse Properties</a> for details.")
}
...
￿    VK_QUEUE_SPARSE_BINDING_BIT = 0x00000008,
		...
		<ul>
			...
			<li>if #QUEUE_COMPUTE_BIT is set, then the queues in this queue family support compute operations.</li>
			<li>if #QUEUE_TRANSFER_BIT is set, then the queues in this queue family support transfer operations.</li>
			<li>if #QUEUE_SPARSE_BINDING_BIT is set, then the queues in this queue family support sparse memory management operations (see <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#sparsememory">Sparse Resources</a>). If any of the sparse resource features are enabled, then at least one queue family <b>must</b> support this bit.</li>
		</ul>
		...
		For further details see <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#devsandqueues-queues">Queues</a>.
...
￿    VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT = 0x00000010,
		...
		<ul>
			...
			<li>if {@code propertyFlags} has the #MEMORY_PROPERTY_HOST_VISIBLE_BIT bit set, memory allocated with this type <b>can</b> be mapped for host access using #MapMemory().</li>
			<li>if {@code propertyFlags} has the #MEMORY_PROPERTY_HOST_COHERENT_BIT bit set, host cache management commands #FlushMappedMemoryRanges() and #InvalidateMappedMemoryRanges() are not needed to make host writes visible to the device or device writes visible to the host, respectively.</li>
			<li>if {@code propertyFlags} has the #MEMORY_PROPERTY_HOST_CACHED_BIT bit set, memory allocated with this type is cached on the host. Host memory accesses to uncached memory are slower than to cached memory, however uncached memory is always host coherent.</li>
			<li>if {@code propertyFlags} has the #MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT bit set, the memory type only allows device access to the memory. Memory types <b>must</b> not have both #MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT and #MEMORY_PROPERTY_HOST_VISIBLE_BIT set. Additionally, the object&#8217;s backing memory <b>may</b> be provided by the implementation lazily as specified in <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#memory-device-lazy_allocation">Lazily Allocated Memory</a>.</li>
		</ul>""")
...
val VkPhysicalDeviceMemoryProperties = struct(VULKAN_PACKAGE, "VkPhysicalDeviceMemoryProperties", mutable = false) {
	...
	documentation =
		...
		At least one heap <b>must</b> include #MEMORY_HEAP_DEVICE_LOCAL_BIT in ##VkMemoryHeap{@code ::flags}. If there are multiple heaps that all have similar performance characteristics, they <b>may</b> all include #MEMORY_HEAP_DEVICE_LOCAL_BIT. In a unified memory architecture (UMA) system, there is often only a single memory heap which is considered to be equally “local” to the host and to the device, and such an implementation <b>must</b> advertise the heap as device-local.
...
val VkDeviceQueueCreateInfo = struct(VULKAN_PACKAGE, "VkDeviceQueueCreateInfo") {
	...
	const..float_p.member("pQueuePriorities", "an array of {@code queueCount} normalized floating point values, specifying priorities of work that will be submitted to each created queue. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#devsandqueues-priority\">Queue Priority</a> for more information.")
}
...
val VkDeviceCreateInfo = struct(VULKAN_PACKAGE, "VkDeviceCreateInfo") {
	...
	const..charUTF8_p_const_p.member("ppEnabledLayerNames", "deprecated and ignored. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#extended-functionality-device-layer-deprecation\">Device Layer Deprecation</a>.")
	...
	const..charUTF8_p_const_p.member("ppEnabledExtensionNames", "a pointer to an array of {@code enabledExtensionCount} null-terminated UTF-8 strings containing the names of extensions to enable for the created device. See the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#extended-functionality-extensions\">Extensions</a> section for further details.")
	nullable..const..VkPhysicalDeviceFeatures.p.member("pEnabledFeatures", "{@code NULL} or a pointer to a ##VkPhysicalDeviceFeatures structure that contains boolean indicators of all the features to be enabled. Refer to the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#features-features\">Features</a> section for further details.")
}
...
val VkLayerProperties = struct(VULKAN_PACKAGE, "VkLayerProperties", mutable = false) {
	...
	uint32_t.member("specVersion", "the Vulkan version the layer was written to, encoded as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fundamentals-versionnum\">API Version Numbers and Semantics</a> section.")
...
val VkSubmitInfo = struct(VULKAN_PACKAGE, "VkSubmitInfo") {
	documentation =
		...
		<ul>
			...
			<li>If any given element of {@code pCommandBuffers} was recorded with #COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, it <b>must</b> not have been previously submitted without re-recording that command buffer</li>
			<li>If any given element of {@code pCommandBuffers} contains commands that execute secondary command buffers recorded with #COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, each such secondary command buffer <b>must</b> not have been previously submitted without re-recording that command buffer</li>
	...
	const..VkCommandBuffer.p.member("pCommandBuffers", "a pointer to an array of command buffers to execute in the batch. The command buffers submitted in a batch begin execution in the order they appear in {@code pCommandBuffers}, but <b>may</b> complete out of order.")
...
val VkMemoryAllocateInfo = struct(VULKAN_PACKAGE, "VkMemoryAllocateInfo") {
	documentation =
		...
		<ul>
			...
			<li>{@code pNext} <b>must</b> be {@code NULL}, or a pointer to a valid instance of ##VkDedicatedAllocationMemoryAllocateInfoNV</li>
		</ul>
...
val VkMappedMemoryRange = struct(VULKAN_PACKAGE, "VkMappedMemoryRange") {
	documentation =
		...
		<ul>
			...
			<li>If {@code size} is not equal to #WHOLE_SIZE, {@code size} <b>must</b> be a multiple of ##VkPhysicalDeviceLimits{@code ::nonCoherentAtomSize}</li>
		</ul>
...
￿    VK_SPARSE_IMAGE_FORMAT_NONSTANDARD_BLOCK_SIZE_BIT = 0x00000004,
		...
		<ul>
			...
			<li>If #SPARSE_IMAGE_FORMAT_ALIGNED_MIP_SIZE_BIT is set, the first mip level whose dimensions are not integer multiples of the corresponding dimensions of the sparse image block begins the mip tail region.</li>
			<li>If #SPARSE_IMAGE_FORMAT_NONSTANDARD_BLOCK_SIZE_BIT is set, the image uses non-standard sparse image block dimensions, and the {@code imageGranularity} values do not match the standard sparse image block dimensions for the given pixel format.</li>
		</ul>""")
}
...
val VkSparseMemoryBind = struct(VULKAN_PACKAGE, "VkSparseMemoryBind") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If {@code memory} is not #NULL_HANDLE, {@code memory} and {@code memoryOffset} <b>must</b> match the memory requirements of the resource, as described in section <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#resources-association">the “Resource Memory Association” section</a></li>
...
val VkQueryPoolCreateInfo = struct(VULKAN_PACKAGE, "VkQueryPoolCreateInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-pipelineStatisticsQuery">pipeline statistics queries</a> feature is not enabled, {@code queryType} <b>must</b> not be #QUERY_TYPE_PIPELINE_STATISTICS</li>
...
￿    VK_QUERY_TYPE_TIMESTAMP = 2,
	...
	VkQueryPipelineStatisticFlags.member("pipelineStatistics", "a bitmask indicating which counters will be returned in queries on the new pool, as described below in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#queries-pipestats\">the “Pipeline Statistics Queries” section</a>. {@code pipelineStatistics} is ignored if {@code queryType} is not #QUERY_TYPE_PIPELINE_STATISTICS.")
}
...
￿    VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT = 0x00000100,
		...
		<ul>
			...
			<li>#BUFFER_USAGE_UNIFORM_BUFFER_BIT indicates that the buffer <b>can</b> be used in a ##VkDescriptorBufferInfo suitable for occupying a {@code VkDescriptorSet} slot either of type #DESCRIPTOR_TYPE_UNIFORM_BUFFER or #DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC.</li>
...
￿    VK_BUFFER_CREATE_SPARSE_ALIASED_BIT = 0x00000004,
		...
		<ul>
			<li>#BUFFER_CREATE_SPARSE_BINDING_BIT indicates that the buffer will be backed using sparse memory binding.</li>
			<li>#BUFFER_CREATE_SPARSE_RESIDENCY_BIT indicates that the buffer <b>can</b> be partially backed using sparse memory binding. Buffers created with this flag <b>must</b> also be created with the #BUFFER_CREATE_SPARSE_BINDING_BIT flag.</li>
			<li>#BUFFER_CREATE_SPARSE_ALIASED_BIT indicates that the buffer will be backed using sparse memory binding with memory ranges that might also simultaneously be backing another buffer (or another portion of the same buffer). Buffers created with this flag <b>must</b> also be created with the #BUFFER_CREATE_SPARSE_BINDING_BIT flag.</li>
		</ul>
		...
		<ul>
			...
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-sparseBinding">sparse bindings</a> feature is not enabled, {@code flags} <b>must</b> not contain #BUFFER_CREATE_SPARSE_BINDING_BIT</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-sparseResidencyBuffer">sparse buffer residency</a> feature is not enabled, {@code flags} <b>must</b> not contain #BUFFER_CREATE_SPARSE_RESIDENCY_BIT</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-sparseResidencyAliased">sparse aliased residency</a> feature is not enabled, {@code flags} <b>must</b> not contain #BUFFER_CREATE_SPARSE_ALIASED_BIT</li>
	...
	VkSharingMode.member("sharingMode", "the sharing mode of the buffer when it will be accessed by multiple queue families, see {@code VkSharingMode} in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#resources-sharing\">Resource Sharing</a> section below for supported values.")
...
val VkImageCreateInfo = struct(VULKAN_PACKAGE, "VkImageCreateInfo") {
	documentation =
		...
		<ul>
			...
			<li>{@code pNext} <b>must</b> be {@code NULL}, or a pointer to a valid instance of ##VkDedicatedAllocationImageCreateInfoNV</li>
	...
	VkSampleCountFlagBits.member("samples", "the number of sub-data element samples in the image as defined in {@code VkSampleCountFlagBits}. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#primsrast-multisampling\">Multisampling</a>.")
	VkImageTiling.member("tiling", "a {@code VkImageTiling} specifying the tiling arrangement of the data elements in memory, as described below.")
	VkImageUsageFlags.member("usage", "a bitmask describing the intended usage of the image. See {@code VkImageUsageFlagBits} below for a description of the supported bits.")
	VkSharingMode.member("sharingMode", "the sharing mode of the image when it will be accessed by multiple queue families, and <b>must</b> be one of the values described for {@code VkSharingMode} in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#resources-sharing\">Resource Sharing</a> section below.")
	AutoSize("pQueueFamilyIndices", optional = true)..uint32_t.member("queueFamilyIndexCount", "the number of entries in the {@code pQueueFamilyIndices} array.")
	...
	VkImageLayout.member("initialLayout", "selects the initial {@code VkImageLayout} state of all image subresources of the image. See <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#resources-image-layouts\">Image Layouts</a>. {@code initialLayout} <b>must</b> be #IMAGE_LAYOUT_UNDEFINED or #IMAGE_LAYOUT_PREINITIALIZED.")
}
...
￿    VK_COMPONENT_SWIZZLE_A = 6,
		...
		<ul>
			...
			<li>#COMPONENT_SWIZZLE_ZERO: the component is set to zero.</li>
			...
			<li>#COMPONENT_SWIZZLE_R: the component is set to the value of the R component of the image.</li>
			<li>#COMPONENT_SWIZZLE_G: the component is set to the value of the G component of the image.</li>
			<li>#COMPONENT_SWIZZLE_B: the component is set to the value of the B component of the image.</li>
			<li>#COMPONENT_SWIZZLE_A: the component is set to the value of the A component of the image.</li>
		</ul>
	...
	VkComponentSwizzle.member("r", "determines the component value placed in the R component of the output vector.")
	VkComponentSwizzle.member("g", "determines the component value placed in the G component of the output vector.")
	VkComponentSwizzle.member("b", "determines the component value placed in the B component of the output vector.")
	VkComponentSwizzle.member("a", "determines the component value placed in the A component of the output vector.")
}
...
￿    VK_IMAGE_ASPECT_METADATA_BIT = 0x00000008,
	...
	VkImageAspectFlags.member("aspectMask", "a bitmask indicating which aspect(s) of the image are included in the view. See {@code VkImageAspectFlagBits}.")
...
val VkImageViewCreateInfo = struct(VULKAN_PACKAGE, "VkImageViewCreateInfo") {
	documentation =
		...
		<table class="lwjgl">
			...
			<tbody>
				<tr><td></td><td>{@code imageType} = ci.{@code imageType} {@code width} = ci.{@code extent.width} {@code height} = ci.{@code extent.height} {@code depth} = ci.{@code extent.depth} {@code arrayLayers} = ci.{@code arrayLayers} {@code samples} = ci.{@code samples} where ci is the ##VkImageCreateInfo used to create {@code image}.</td><td>{@code baseArrayLayer} and {@code layerCount} are members of the {@code subresourceRange} member.</td></tr>
				<tr><td>1D, 0, 0</td><td>{@code imageType} = #IMAGE_TYPE_1D {@code width} &#x2265; 1 {@code height} = 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} = 1</td><td>{@code viewType} = #VIEW_TYPE_1D {@code baseArrayLayer} &#x2265; 0 {@code layerCount} = 1</td></tr>
				<tr><td>1D, 1, 0</td><td>{@code imageType} = #IMAGE_TYPE_1D {@code width} &#x2265; 1 {@code height} = 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} = 1</td><td>{@code viewType} = #VIEW_TYPE_1D_ARRAY {@code baseArrayLayer} &#x2265; 0 {@code layerCount} &#x2265; 1</td></tr>
				<tr><td>2D, 0, 0</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} &#x2265; 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} = 1</td><td>{@code viewType} = #VIEW_TYPE_2D {@code baseArrayLayer} &#x2265; 0 {@code layerCount} = 1</td></tr>
				<tr><td>2D, 1, 0</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} &#x2265; 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} = 1</td><td>{@code viewType} = #VIEW_TYPE_2D_ARRAY {@code baseArrayLayer} &#x2265; 0 {@code layerCount} &#x2265; 1</td></tr>
				<tr><td>2D, 0, 1</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} &#x2265; 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} &gt; 1</td><td>{@code viewType} = #VIEW_TYPE_2D {@code baseArrayLayer} &#x2265; 0 {@code layerCount} = 1</td></tr>
				<tr><td>2D, 1, 1</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} &#x2265; 1 {@code depth} = 1 {@code arrayLayers} &#x2265; 1 {@code samples} &gt; 1</td><td>{@code viewType} = #VIEW_TYPE_2D_ARRAY {@code baseArrayLayer} &#x2265; 0 {@code layerCount} &#x2265; 1</td></tr>
				<tr><td>CUBE, 0, 0</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} = {@code width} {@code depth} = 1 {@code arrayLayers} &#x2265; 6 {@code samples} = 1 {@code flags} includes #IMAGE_CREATE_CUBE_COMPATIBLE_BIT</td><td>{@code viewType} = #VIEW_TYPE_CUBE {@code baseArrayLayer} &#x2265; 0 {@code layerCount} = 6</td></tr>
				<tr><td>CUBE, 1, 0</td><td>{@code imageType} = #IMAGE_TYPE_2D {@code width} &#x2265; 1 {@code height} = width {@code depth} = 1 <em>N</em> &#x2265; 1 {@code arrayLayers} &#x2265; 6 &#x00d7; <em>N</em> {@code samples} = 1 {@code flags} includes #IMAGE_CREATE_CUBE_COMPATIBLE_BIT</td><td>{@code viewType} = #VIEW_TYPE_CUBE_ARRAY {@code baseArrayLayer} &#x2265; 0 {@code layerCount} = 6 &#x00d7; <em>N</em>, <em>N</em> &#x2265; 1</td></tr>
				<tr><td>3D, 0, 0</td><td>{@code imageType} = #IMAGE_TYPE_3D {@code width} &#x2265; 1 {@code height} &#x2265; 1 {@code depth} &#x2265; 1 {@code arrayLayers} = 1 {@code samples} = 1</td><td>{@code viewType} = #VIEW_TYPE_3D {@code baseArrayLayer} = 0 {@code layerCount} = 1</td></tr>
			</tbody>
		</table>
		...
		<ul>
			...
			<li>If {@code image} was created with #IMAGE_TILING_OPTIMAL and {@code usage} containing #IMAGE_USAGE_SAMPLED_BIT, {@code format} <b>must</b> be supported for sampled images, as specified by the #FORMAT_FEATURE_SAMPLED_IMAGE_BIT flag in ##VkFormatProperties{@code ::optimalTilingFeatures} returned by #GetPhysicalDeviceFormatProperties() with the same value of {@code format}</li>
			<li>If {@code image} was created with #IMAGE_TILING_OPTIMAL and {@code usage} containing #IMAGE_USAGE_STORAGE_BIT, {@code format} <b>must</b> be supported for storage images, as specified by the #FORMAT_FEATURE_STORAGE_IMAGE_BIT flag in ##VkFormatProperties{@code ::optimalTilingFeatures} returned by #GetPhysicalDeviceFormatProperties() with the same value of {@code format}</li>
			<li>If {@code image} was created with #IMAGE_TILING_OPTIMAL and {@code usage} containing #IMAGE_USAGE_COLOR_ATTACHMENT_BIT, {@code format} <b>must</b> be supported for color attachments, as specified by the #FORMAT_FEATURE_COLOR_ATTACHMENT_BIT flag in ##VkFormatProperties{@code ::optimalTilingFeatures} returned by #GetPhysicalDeviceFormatProperties() with the same value of {@code format}</li>
	...
	VkComponentMapping.member("components", "specifies a remapping of color components (or of depth or stencil components after they have been converted into color components). See ##VkComponentMapping.")
...
val VkPipelineShaderStageCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineShaderStageCreateInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-geometryShader">geometry shaders</a> feature is not enabled, {@code stage} <b>must</b> not be #SHADER_STAGE_GEOMETRY_BIT</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-tessellationShader">tessellation shaders</a> feature is not enabled, {@code stage} <b>must</b> not be #SHADER_STAGE_TESSELLATION_CONTROL_BIT or #SHADER_STAGE_TESSELLATION_EVALUATION_BIT</li>
...
￿    VK_SHADER_STAGE_ALL = 0x7FFFFFFF,
	...
	nullable..const..VkSpecializationInfo.p.member("pSpecializationInfo", "a pointer to ##VkSpecializationInfo, as described in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#pipelines-specialization-constants\">Specialization Constants</a>, and <b>can</b> be {@code NULL}.")
}
...
￿    VK_VERTEX_INPUT_RATE_INSTANCE = 1,
		...
		<ul>
			...
			<li>#VERTEX_INPUT_RATE_INSTANCE indicates that vertex attribute addressing is a function of the instance index.</li>
		</ul>""")
}
...
val VkPipelineInputAssemblyStateCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineInputAssemblyStateCreateInfo") {
	documentation =
		...
		<ul>
			...
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-tessellationShader">tessellation shaders</a> feature is not enabled, {@code topology} <b>must</b> not be #PRIMITIVE_TOPOLOGY_PATCH_LIST</li>
		</ul>
...
val VkPipelineRasterizationStateCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineRasterizationStateCreateInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-depthClamp">depth clamping</a> feature is not enabled, {@code depthClampEnable} <b>must</b> be #FALSE</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-fillModeNonSolid">non-solid fill modes</a> feature is not enabled, {@code polygonMode} <b>must</b> be #POLYGON_MODE_FILL</li>
		</ul>
	...
	VkFrontFace.member("frontFace", "the front-facing triangle orientation to be used for culling. See {@code VkFrontFace}.")
...
val VkPipelineMultisampleStateCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineMultisampleStateCreateInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-sampleRateShading">sample rate shading</a> feature is not enabled, {@code sampleShadingEnable} <b>must</b> be #FALSE</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-alphaToOne">alpha to one</a> feature is not enabled, {@code alphaToOneEnable} <b>must</b> be #FALSE</li>
	...
	float.member("minSampleShading", "the minimum fraction of sample shading, as described in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#primsrast-sampleshading\">Sample Shading</a>.")
	nullable..const..VkSampleMask.p.member("pSampleMask", "a bitmask of static coverage information that is ANDed with the coverage information generated during rasterization, as described in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-samplemask\">Sample Mask</a>.")
	VkBool32.member("alphaToCoverageEnable", "controls whether a temporary coverage value is generated based on the alpha component of the fragment&#8217;s first color output as specified in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-covg\">Multisample Coverage</a> section.")
	VkBool32.member("alphaToOneEnable", "controls whether the alpha component of the fragment&#8217;s first color output is replaced with one as described in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-covg\">Multisample Coverage</a>.")
}
...
val VkPipelineDepthStencilStateCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineDepthStencilStateCreateInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-depthBounds">depth bounds testing</a> feature is not enabled, {@code depthBoundsTestEnable} <b>must</b> be #FALSE</li>
		</ul>
	...
	VkCompareOp.member("depthCompareOp", "the comparison operator used in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-depth\">depth test</a>.")
	VkBool32.member("depthBoundsTestEnable", "controls whether <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-dbt\">depth bounds testing</a> is enabled.")
	VkBool32.member("stencilTestEnable", "controls whether <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-stencil\">stencil testing</a> is enabled.")
	VkStencilOpState.member("front", "{@code front} and {@code back} control the parameters of the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-stencil\">stencil test</a>.")
	VkStencilOpState.member("back", "see {@code front}")
	float.member("minDepthBounds", "{@code minDepthBounds} and {@code maxDepthBounds} define the range of values used in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#fragops-dbt\">depth bounds test</a>.")
...
val VkPipelineColorBlendAttachmentState = struct(VULKAN_PACKAGE, "VkPipelineColorBlendAttachmentState") {
	...
	VkColorComponentFlags.member("colorWriteMask", "a bitmask selecting which of the R, G, B, and/or A components are enabled for writing, as described later in this chapter.")
}
...
val VkPipelineColorBlendStateCreateInfo = struct(VULKAN_PACKAGE, "VkPipelineColorBlendStateCreateInfo") {
	documentation =
		...
		<ul>
			...
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-logicOp">logic operations</a> feature is not enabled, {@code logicOpEnable} <b>must</b> be #FALSE</li>
	...
	float.array("blendConstants", "an array of four values used as the R, G, B, and A components of the blend constant that are used in blending, depending on the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#framebuffer-blendfactors\">blend factor</a>.", size = 4)
}
...
val VkGraphicsPipelineCreateInfo = struct(VULKAN_PACKAGE, "VkGraphicsPipelineCreateInfo") {
	documentation =
		...
		<h5>Description</h5>
		The parameters {@code basePipelineHandle} and {@code basePipelineIndex} are described in more detail in <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#pipelines-pipeline-derivatives">Pipeline Derivatives</a>.

		{@code pStages} points to an array of ##VkPipelineShaderStageCreateInfo structures, which were previously described in <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#pipelines-compute">Compute Pipelines</a>.
...
￿    VK_PIPELINE_CREATE_DERIVATIVE_BIT = 0x00000004,
		...
		<ul>
			...
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineIndex} is not {@code -1}, it <b>must</b> be a valid index into the calling command&#8217;s {@code pCreateInfos} parameter</li>
			...
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineHandle} is not #NULL_HANDLE, {@code basePipelineHandle} <b>must</b> be a valid {@code VkPipeline} handle</li>
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineHandle} is not #NULL_HANDLE, it <b>must</b> be a valid handle to a graphics {@code VkPipeline}</li>
			...
			<li>If {@code pStages} includes a geometry shader stage, and does not include any tessellation shader stages, its shader code <b>must</b> contain an {@code OpExecutionMode} instruction that specifies an input primitive type that is <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#shaders-geometry-execution">compatible</a> with the primitive topology specified in {@code pInputAssembly}</li>
			<li>If {@code pStages} includes a geometry shader stage, and also includes tessellation shader stages, its shader code <b>must</b> contain an {@code OpExecutionMode} instruction that specifies an input primitive type that is <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#shaders-geometry-execution">compatible</a> with the primitive topology that is output by the tessellation stages</li>
			...
			<li>{@code layout} <b>must</b> be <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#descriptorsets-pipelinelayout-consistency">consistent</a> with all shaders specified in {@code pStages}</li>
...
val VkComputePipelineCreateInfo = struct(VULKAN_PACKAGE, "VkComputePipelineCreateInfo") {
	documentation =
		...
		<ul>
			...
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineHandle} is not #NULL_HANDLE, {@code basePipelineIndex} <b>must</b> be {@code -1}</li>
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineHandle} is not #NULL_HANDLE, {@code basePipelineHandle} <b>must</b> be a valid {@code VkPipeline} handle</li>
			<li>If {@code flags} contains the #PIPELINE_CREATE_DERIVATIVE_BIT flag, and {@code basePipelineHandle} is not #NULL_HANDLE, it <b>must</b> be a valid handle to a compute {@code VkPipeline}</li>
			...
			<li>{@code layout} <b>must</b> be <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#descriptorsets-pipelinelayout-consistency">consistent</a> with the layout of the compute shader specified in {@code stage}</li>
		</ul>
...
￿    VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE = 4,
		...
		These values control the behavior of sampling with coordinates outside the range <code>[0,1]</code> for the respective u, v, or w coordinate as defined in the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#textures-wrapping-operation"> Wrapping Operation</a> section.

		<ul>
			<li>#SAMPLER_ADDRESS_MODE_REPEAT indicates that the repeat wrap mode will be used.</li>
			<li>#SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT indicates that the mirrored repeat wrap mode will be used.</li>
			<li>#SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE indicates that the clamp to edge wrap mode will be used.</li>
			<li>#SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER indicates that the clamp to border wrap mode will be used.</li>
			<li>#SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE indicates that the mirror clamp to edge wrap mode will be used. This is only valid if the {@code VK_KHR_mirror_clamp_to_edge} extension is enabled.</li>
		</ul>
		...
		<ul>
			...
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-samplerAnisotropy">anisotropic sampling</a> feature is not enabled, {@code anisotropyEnable} <b>must</b> be #FALSE</li>
			...
			<li>If the {@code VK_KHR_sampler_mirror_clamp_to_edge} extension is not enabled, {@code addressModeU}, {@code addressModeV} and {@code addressModeW} <b>must</b> not be #SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE</li>
...
￿    VK_FILTER_CUBIC_IMG = 1000015000,
	...
	VkSamplerMipmapMode.member("mipmapMode", """the mipmap filter to apply to lookups as described in the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#textures-texel-filtering">Texel Filtering</a> section, and is of type:
...
￿    VK_SAMPLER_MIPMAP_MODE_LINEAR = 1,
	...
	VkBool32.member("anisotropyEnable", "#TRUE to enable anisotropic filtering, as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#textures-texel-anisotropic-filtering\">Texel Anisotropic Filtering</a> section, or #FALSE otherwise.")
...
<ul>
	...
	VkCompareOp.member("compareOp", "the comparison function to apply to fetched data before filtering as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#textures-depth-compare-operation\">Depth Compare Operation</a> section. See {@code VkCompareOp}.")
	float.member("minLod", "{@code minLod} and {@code maxLod} are the values used to clamp the computed level-of-detail value, as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#textures-level-of-detail-operation\">Level-of-Detail Operation</a> section. {@code maxLod} <b>must</b> be greater than or equal to {@code minLod}.")
	float.member("maxLod", "see {@code minLod}")
	VkBorderColor.member("borderColor", """the predefined border color to use, as described in the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#textures-texel-replacement">Texel Replacement</a> section, and is of type:
...
val VkDescriptorImageInfo = struct(VULKAN_PACKAGE, "VkDescriptorImageInfo") {
	...
	VkImageLayout.member("imageLayout", "the layout that the image will be in at the time this descriptor is accessed. {@code imageLayout} is used in descriptor updates for types #DESCRIPTOR_TYPE_SAMPLED_IMAGE, #DESCRIPTOR_TYPE_STORAGE_IMAGE, #DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, and #DESCRIPTOR_TYPE_INPUT_ATTACHMENT.")
}
...
<div style="margin-left: 26px; border-left: 1px solid gray; padding-left: 14px;"><h5>Note</h5>
		When using #WHOLE_SIZE, the effective range <b>must</b> not be larger than the maximum range for the descriptor type (<a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-limits-maxUniformBufferRange"> maxUniformBufferRange</a> or <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-limits-maxStorageBufferRange"> maxStorageBufferRange</a>). This means that #WHOLE_SIZE is not typically useful in the common case where uniform buffer descriptors are suballocated from a buffer that is much larger than {@code maxUniformBufferRange}.
		</div>
...
val VkWriteDescriptorSet = struct(VULKAN_PACKAGE, "VkWriteDescriptorSet") {
	documentation =
		...
		<h5>See Also</h5>
		##VkDescriptorBufferInfo, ##VkDescriptorImageInfo, #UpdateDescriptorSets()
		"""
	...
	nullable..const..VkBufferView.p.member("pTexelBufferView", "points to an array of {@code VkBufferView} handles as described in the <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#resources-buffer-views\">Buffer Views</a> section or is ignored, as described below.")
}
...
val VkFramebufferCreateInfo = struct(VULKAN_PACKAGE, "VkFramebufferCreateInfo") {
	...
	uint32_t.member("width", "{@code width}, {@code height} and {@code layers} define the dimensions of the framebuffer.")
...
￿    VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT = 0x00000002,
		...
		<ul>
			...
			<li>#COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT controls whether command buffers allocated from the pool <b>can</b> be individually reset. If this flag is set, individual command buffers allocated from the pool <b>can</b> be reset either explicitly, by calling #ResetCommandBuffer(), or implicitly, by calling #BeginCommandBuffer() on an executable command buffer. If this flag is not set, then #ResetCommandBuffer() and #BeginCommandBuffer() (on an executable command buffer) <b>must</b> not be called on the command buffers allocated from the pool, and they <b>can</b> only be reset in bulk by calling #ResetCommandPool().</li>
		</ul>""")
	uint32_t.member("queueFamilyIndex", "designates a queue family as described in section <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#devsandqueues-queueprops\">Queue Family Properties</a>. All command buffers allocated from this command pool <b>must</b> be submitted on queues from the same queue family.")
}
...
val VkCommandBufferInheritanceInfo = struct(VULKAN_PACKAGE, "VkCommandBufferInheritanceInfo") {
	documentation =
		...
		<h5>Valid Usage</h5>
		<ul>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-inheritedQueries">inherited queries</a> feature is not enabled, {@code occlusionQueryEnable} <b>must</b> be #FALSE</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-inheritedQueries">inherited queries</a> feature is enabled, {@code queryFlags} <b>must</b> be a valid combination of {@code VkQueryControlFlagBits} values</li>
			<li>If the <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#features-features-pipelineStatisticsQuery">pipeline statistics queries</a> feature is not enabled, {@code pipelineStatistics} <b>must</b> be 0</li>
		</ul>
...
￿    VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT = 0x00000004,
		...
		<ul>
			...
			<li>#COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT indicates that a secondary command buffer is considered to be entirely inside a render pass. If this is a primary command buffer, then this bit is ignored.</li>
			<li>Setting #COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT allows the command buffer to be resubmitted to a queue or recorded into a primary command buffer while it is pending execution.</li>
		</ul>""")
...
val VkClearAttachment = struct(VULKAN_PACKAGE, "VkClearAttachment") {
	...
	VkClearValue.member("clearValue", "the color or depth/stencil value to clear the attachment to, as described in <a href=\"https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\\#clears-values\">Clear Values</a> below.")
}
...
val VkBufferMemoryBarrier = struct(VULKAN_PACKAGE, "VkBufferMemoryBarrier") {
	documentation =
		...
		<ul>
			...
			<li>If {@code buffer} was created with a sharing mode of #SHARING_MODE_CONCURRENT, {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} <b>must</b> both be #QUEUE_FAMILY_IGNORED</li>
			<li>If {@code buffer} was created with a sharing mode of #SHARING_MODE_EXCLUSIVE, {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} <b>must</b> either both be #QUEUE_FAMILY_IGNORED, or both be a valid queue family (see <a href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/xhtml/vkspec.html\#devsandqueues-queueprops">the “Queue Family Properties” section</a>)</li>
			<li>If {@code buffer} was created with a sharing mode of #SHARING_MODE_EXCLUSIVE, and {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} are valid queue families, at least one of them <b>must</b> be the same as the family of the queue that will execute this barrier</li>
		</ul>
...
val VkImageMemoryBarrier = struct(VULKAN_PACKAGE, "VkImageMemoryBarrier") {
	documentation =
		...
		<ul>
			...
			<li>If {@code image} was created with a sharing mode of #SHARING_MODE_CONCURRENT, {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} <b>must</b> both be #QUEUE_FAMILY_IGNORED</li>
...
val VkDispatchIndirectCommand = struct(VULKAN_PACKAGE, "VkDispatchIndirectCommand") {
	documentation =
		...
		<h5>Description</h5>
		The members of ##VkDispatchIndirectCommand structure have the same meaning as the similarly named parameters of #CmdDispatch().
