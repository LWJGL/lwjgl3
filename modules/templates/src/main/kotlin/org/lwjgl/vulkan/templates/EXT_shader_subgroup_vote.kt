...
val EXT_shader_subgroup_vote = "EXTShaderSubgroupVote".nativeClassVK("EXT_shader_subgroup_vote", postfix = EXT) {
	documentation =
		"""
		<dl>
			...
			<dd><ul>
				...
				<li>This extension requires the https://www.khronos.org/registry/spir-v/extensions/KHR/SPV_KHR_subgroup_vote.html[SPV_KHR_subgroup_vote] SPIR-V extension.</li>
				<li>This extension requires the https://www.opengl.org/registry/specs/ARB/shader_group_vote.txt[GL_ARB_shader_group_vote] extension for GLSL source languages.</li>
			</ul></dd>
...
￿  result = do_general_path();
			...
			The built-in function {@code allInvocationsARB}() will return the same value for all invocations in the group, so the group will either execute {@code do_fast_path}() or {@code do_general_path}(), but never both. For example, shader code might want to evaluate a complex function iteratively by starting with an approximation of the result and then refining the approximation. Some input values may require a small number of iterations to generate an accurate result ({@code do_fast_path}) while others require a larger number ({@code do_general_path}). In another example, shader code might want to evaluate a complex function ({@code do_general_path}) that can be greatly simplified when assuming a specific value for one of its inputs ({@code do_fast_path}).</dd>
		</dl>
		"""
