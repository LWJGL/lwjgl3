...
val ARB_bindless_texture = "ARBBindlessTexture".nativeClassGL("ARB_bindless_texture", postfix = ARB) {
	...
	documentation =
		...
		capability provided by the ${registryLinkTo("NV", "shader_buffer_load")} extension that allows shaders to access buffer objects without binding them. In
		both cases, these extensions significantly reduce the amount of API and internal GL driver overhead needed to manage resource bindings.
		...
		${ARB_shader_image_load_store.link} and ${registryLinkTo("EXT", "shader_image_load_store")} extensions, where a texture can be accessed without first
		...
		To access texture or image resources using handles, the handles must first be made resident. Accessing a texture or image by handle without first making
		...
		a texture resident and non-resident as required.
	...
	GLuint64(
		...
		The error GL11#INVALID_OPERATION is generated if the border color (taken from the embedded sampler for GetTextureHandleARB or from the {@code sampler}
	...
	GLuint64(
		...
		The error GL11#INVALID_VALUE is generated by GetImageHandleARB if:
		${ul(
		...
		The error GL11#INVALID_OPERATION is generated by GetImageHandleARB if:
		${ul(
	...
	GLvoid(
		...
		Check(1) _ returnValue _ GLuint64_p.OUT("params", "a buffer in which to place the returned data")
	)
}