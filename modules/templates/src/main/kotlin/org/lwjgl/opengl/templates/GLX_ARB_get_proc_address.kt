...
val GLX_ARB_get_proc_address = "GLXARBGetProcAddress".nativeClassGLX("GLX_ARB_get_proc_address", ARB) {
	...
	voidptr(
		...
		must must also query #GetString()(GL11##GL_EXTENSIONS) or GLX11#QueryExtensionsString() to determine if an extension is supported by a particular
		context.
		...
		const..GLcharASCII_p.IN("procName", "the function name to query")
	)
}