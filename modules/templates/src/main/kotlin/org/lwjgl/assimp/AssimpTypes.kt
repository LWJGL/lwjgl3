...
val ASSIMP_BINDING_DELEGATE = ASSIMP_BINDING.delegate("Assimp.getLibrary()")

val MAXLEN = 1024

val aiPlane = struct(ASSIMP_PACKAGE, "AIPlane", nativeName = "aiPlane") {
...
val aiVector2D = struct(ASSIMP_PACKAGE, "AIVector2D", nativeName = "aiVector2D") {
    float.member("x", "")
    float.member("y", "")
}

val aiVector3D = struct(ASSIMP_PACKAGE, "AIVector3D", nativeName = "aiVector3D") {
    float.member("x", "")
    float.member("y", "")
    float.member("z", "")
}
...
val aiRay = struct(ASSIMP_PACKAGE, "AIRay", nativeName = "aiRay") {
...
val aiColor3D = struct(ASSIMP_PACKAGE, "AIColor3D", nativeName = "aiColor3D") {
...
val aiString = struct(ASSIMP_PACKAGE, "AIString", nativeName = "aiString") {
    ...
    charUTF8.array("data", "String buffer.", size = MAXLEN)
}

val aiString_p = aiString.p

val aiMemoryInfo = struct(ASSIMP_PACKAGE, "AIMemoryInfo", nativeName = "aiMemoryInfo", mutable = false) {
...
val aiTexel_p = aiTexel.p;

val aiTexture = struct(ASSIMP_PACKAGE, "AITexture", nativeName = "aiTexture") {
    documentation = """
        ...
        ${ul(
            ...
            "Compressed textures stored in a file format like png or jpg. The raw file"
        )}
...
val aiColor4D = struct(ASSIMP_PACKAGE, "AIColor4D", nativeName = "aiColor4D") {
...
val aiColor4D_p = aiColor4D.p

val aiMatrix4x4 = struct(ASSIMP_PACKAGE, "AIMatrix4x4", nativeName = "aiMatrix4x4") {
...
val aiMetadataEntry = struct(ASSIMP_PACKAGE, "AIMetaDataEntry", nativeName = "aiMetadataEntry") {
    aiMetadataType.member("mType", "")
    void_p.member("mData", "")
}

val aiMetadataEntry_p = aiMetadataEntry.p

val aiMetadata = struct(ASSIMP_PACKAGE, "AIMetaData", nativeName = "aiMetadata") {
    AutoSize("mKeys", "mValues")..unsigned_int.member("mNumProperties", "Length of the mKeys and mValues arrays, respectively")
    aiString_p.buffer("mKeys", "Arrays of keys, may not be NULL. Entries in this array may not be NULL as well.")
    aiMetadataEntry_p.buffer("mValues", "Arrays of values, may not be NULL. Entries in this array may be NULL if the corresponding property key has no assigned value.")
}
val aiMetadata_p = aiMetadata.p

val aiNode_p = struct(ASSIMP_PACKAGE, "AINode", nativeName = "aiNode").p
val aiNode_pp = aiNode_p.p

val aiNode = struct(ASSIMP_PACKAGE, "AINode", nativeName = "aiNode") {
    documentation = """
...
val aiFace = struct(ASSIMP_PACKAGE, "AIFace", nativeName = "aiFace") {
    documentation = """
    ...
    AutoSize("mIndices")..unsigned_int.member("mNumIndices", "Number of indices defining this face. The maximum value for this member is #AI_MAX_FACE_INDICES.")
    unsigned_int_p.member("mIndices", "Pointer to the indices array. Size of the array is given in numIndices.")
}
val aiFace_p = aiFace.p

val aiVertexWeight = struct(ASSIMP_PACKAGE, "AIVertexWeight", nativeName = "aiVertexWeight") {
...
val aiVertexWeight_p = aiVertexWeight.p

val aiBone = struct(ASSIMP_PACKAGE, "AIBone", nativeName = "aiBone") {
    documentation = """
...
val aiBone_pp = aiBone.p.p

val aiAnimMesh = struct(ASSIMP_PACKAGE, "AIAnimMesh", nativeName = "aiAnimMesh") {
    documentation = """
    ...
    nullable..aiVector3D_p.buffer("mNormals", "Replacement for aiMesh::mNormals.")
    nullable..aiVector3D_p.buffer("mTangents", "Replacement for aiMesh::mTangents.")
    nullable..aiVector3D_p.buffer("mBitangents", "Replacement for aiMesh::mBitangents.")
    nullable..aiColor4D_p.array("mColors", "Replacement for aiMesh::mColors", size = "Assimp.AI_MAX_NUMBER_OF_COLOR_SETS")
    nullable..aiVector3D_p.array("mTextureCoords", "Replacement for aiMesh::mTextureCoords", size = "Assimp.AI_MAX_NUMBER_OF_TEXTURECOORDS")
    AutoSize("mVertices", "mNormals", "mTangents", "mBitangents", "mColors", "mTextureCoords")..unsigned_int.member("mNumVertices", "The number of vertices in the aiAnimMesh, and thus the length of all the member arrays. This has always the same value as the mNumVertices property in the corresponding aiMesh. It is duplicated here merely to make the length of the member arrays accessible even if the aiMesh is not known, e.g. from language bindings.")
}
val aiAnimMesh_pp = aiAnimMesh.p.p

val aiMesh = struct(ASSIMP_PACKAGE, "AIMesh", nativeName = "aiMesh") {
    documentation = """
    ...
    unsigned_int.member("mPrimitiveTypes", """Bitwise combination of the members of the #aiPrimitiveType enum. This specifies which types of primitives are present in the mesh. The "SortByPrimitiveType"-Step can be used to make sure the output meshes consist of one primitive type each.""")
    AutoSize("mVertices", "mNormals", "mTangents", "mBitangents", "mColors", "mTextureCoords")..unsigned_int.member("mNumVertices", "The number of vertices in this mesh. This is also the size of all of the per-vertex data arrays. The maximum value for this member is #AI_MAX_VERTICES.")
    AutoSize("mFaces")..unsigned_int.member("mNumFaces", "The number of primitives (triangles, polygons, lines) in this  mesh. This is also the size of the mFaces array. The maximum value for this member is #AI_MAX_FACES.")
    nullable..aiVector3D_p.buffer("mVertices", "Vertex positions. This array is always present in a mesh. The array is mNumVertices in size.")
    nullable..aiVector3D_p.buffer("mNormals", "Vertex normals. The array contains normalized vectors, NULL if not present. The array is mNumVertices in size. Normals are undefined for point and line primitives. A mesh consisting of points and lines only may not have normal vectors. Meshes with mixed primitive types (i.e. lines and triangles) may have normals, but the normals for vertices that are only referenced by point or line primitives are undefined and set to QNaN")
    ...
    nullable..aiVector3D_p.buffer("mBitangents", "Vertex bitangents. The bitangent of a vertex points in the direction of the positive Y texture axis. The array contains normalized vectors, NULL if not present. The array is mNumVertices in size.")
    nullable..aiColor4D_p.array("mColors", "Vertex color sets. A mesh may contain 0 to #AI_MAX_NUMBER_OF_COLOR_SETS vertex colors per vertex. NULL if not present. Each array is mNumVertices in size if present.", size = "Assimp.AI_MAX_NUMBER_OF_COLOR_SETS")
    nullable..aiVector3D_p.array("mTextureCoords", "Vertex texture coords, also known as UV channels. A mesh may contain 0 to AI_MAX_NUMBER_OF_TEXTURECOORDS per vertex. NULL if not present. The array is mNumVertices in size.", size = "Assimp.AI_MAX_NUMBER_OF_TEXTURECOORDS")
    unsigned_int.array("mNumUVComponents", "Specifies the number of components for a given UV channel. Up to three channels are supported (UVW, for accessing volume or cube maps). If the value is 2 for a given channel n, the component p.z of mTextureCoords[n][p] is set to 0.0f. If the value is 1 for a given channel, p.y is set to 0.0f, too.", size = "Assimp.AI_MAX_NUMBER_OF_TEXTURECOORDS")
    aiFace_p.buffer("mFaces", "The faces the mesh is constructed from. Each face refers to a number of vertices by their indices. This array is always present in a mesh, its size is given in mNumFaces. If the #AI_SCENE_FLAGS_NON_VERBOSE_FORMAT is NOT set each face references an unique set of vertices.")
    AutoSize("mBones")..unsigned_int.member("mNumBones", "The number of bones this mesh contains. Can be 0, in which case the mBones array is NULL.")
    nullable..aiBone_pp.member("mBones", "The bones of this mesh. A bone consists of a name by which it can be found in the frame hierarchy and a set of vertex weights.")
    unsigned_int.member("mMaterialIndex", "The material used by this mesh. A mesh uses only a single material. If an imported model uses multiple materials, the import splits up the mesh. Use this value as index into the scene's material list.")
    aiString.member("mName", "Name of the mesh. Meshes can be named, but this is not a  requirement and leaving this field empty is totally fine. There are mainly three uses for mesh names: - some formats name nodes and meshes independently. - importers tend to split meshes up to meet the one-material-per-mesh requirement. Assigning the same (dummy) name to each of the result meshes aids the caller at recovering the original mesh partitioning. - Vertex animations refer to meshes by their names.")
    AutoSize("mAnimMeshes")..unsigned_int.member("mNumAnimMeshes", "NOT CURRENTLY IN USE. The number of attachment meshes")
    nullable..aiAnimMesh_pp.member("mAnimMeshes", "NOT CURRENTLY IN USE. Attachment meshes for this mesh, for vertex-based animation. Attachment meshes carry replacement data for some of the mesh'es vertex components (usually positions, normals).")
}
val aiMesh_pp = aiMesh.p.p

val aiUVTransform = struct(ASSIMP_PACKAGE, "AIUVTransform", "aiUVTransform") {
    documentation = """
    ...
    float.member("mRotation", "Rotation - in counter-clockwise direction. The rotation angle is specified in radians. The rotation center is 0.5f|0.5f. The default value 0.f.")
}
val aiUVTransform_p = aiUVTransform.p

val aiPropertyTypeInfo = "aiPropertyTypeInfo".enumType
val aiMaterialProperty = struct(ASSIMP_PACKAGE, "AIMaterialProperty", "aiMaterialProperty") {
    documentation = """
        Data structure for a single material property

        As an user, you'll probably never need to deal with this data structure.
        Just use the provided aiGetMaterialXXX() family of functions to query
        material properties easily. Processing them manually is faster, but it
        is not the recommended way. It isn't worth the effort.
...
~<name>
    ...
    unsigned_int.member("mSemantic", "Textures: Specifies their exact usage semantic. For non-texture properties, this member is always 0 (or, better-said, #aiTextureType_NONE).")
    ...
    AutoSize("mData")..unsigned_int.member("mDataLength", "Size of the buffer mData is pointing to, in bytes. This value may not be 0.")
    aiPropertyTypeInfo.member("mType", "Type information for the property. Defines the data layout inside the data buffer. This is used by the library internally to perform debug checks and to utilize proper type conversions. (It's probably a hacky solution, but it works.)")
    char_p.member("mData", "Binary buffer to hold the property's value. The size of the buffer is always mDataLength.")
}
val aiMaterialProperty_pp = aiMaterialProperty.p.p

val aiMaterial = struct(ASSIMP_PACKAGE, "AIMaterial", nativeName = "aiMaterial") {
    documentation = """
...
val aiMaterial_p = aiMaterial.p
val aiMaterial_pp = aiMaterial_p.p

val aiQuaternion = struct(ASSIMP_PACKAGE, "AIQuaternion", nativeName = "aiQuaternion") {
...
val aiVectorKey_p = aiVectorKey.p

val aiQuatKey = struct(ASSIMP_PACKAGE, "AIQuatKey", nativeName = "aiQuatKey") {
...
val aiQuatKey_p = aiQuatKey.p

val aiMeshKey = struct(ASSIMP_PACKAGE, "AIMeshKey", nativeName = "aiMeshKey") {
    documentation = "Binds a anim mesh to a specific point in time."

    double.member("mTime", "The time of this key")
    unsigned_int.member("mValue", "Index into the aiMesh::mAnimMeshes array of the mesh coresponding to the #aiMeshAnim hosting this key frame. The referenced anim mesh is evaluated according to the rules defined in the docs for #aiAnimMesh.")
}
val aiMeshKey_p = aiMeshKey.p

val aiAnimBehaviour = "aiAnimBehaviour".enumType

val aiNodeAnim = struct(ASSIMP_PACKAGE, "AINodeAnim", nativeName = "aiNodeAnim") {
    documentation = """
    ...
    aiVectorKey_p.buffer("mPositionKeys", "The position keys of this animation channel. Positions are specified as 3D vector. The array is mNumPositionKeys in size. If there are position keys, there will also be at least one scaling and one rotation key.")
    AutoSize("mRotationKeys")..unsigned_int.member("mNumRotationKeys", "The number of rotation keys")
    aiQuatKey_p.buffer("mRotationKeys", "The rotation keys of this animation channel. Rotations are given as quaternions,  which are 4D vectors. The array is mNumRotationKeys in size. If there are rotation keys, there will also be at least one scaling and one position key.")
    AutoSize("mScalingKeys")..unsigned_int.member("mNumScalingKeys", "The number of scaling keys")
    aiVectorKey_p.buffer("mScalingKeys", "The scaling keys of this animation channel. Scalings are specified as 3D vector. The array is mNumScalingKeys in size. If there are scaling keys, there will also be at least one position and one rotation key.")
    aiAnimBehaviour.member("mPreState", "Defines how the animation behaves before the first key is encountered. The default value is aiAnimBehaviour_DEFAULT (the original transformation matrix of the affected node is used).")
    aiAnimBehaviour.member("mPostState", "Defines how the animation behaves after the last key was processed. The default value is aiAnimBehaviour_DEFAULT (the original transformation matrix of the affected node is taken).")
}
val aiNodeAnim_pp = aiNodeAnim.p.p

val aiMeshAnim = struct(ASSIMP_PACKAGE, "AIMeshAnim", nativeName = "aiMeshAnim") {
    documentation = """
    ...
    aiString.member("mName", "Name of the mesh to be animated. An empty string is not allowed, animated meshes need to be named (not necessarily uniquely, the name can basically serve as wildcard to select a group of meshes with similar animation setup)")
    AutoSize("mKeys")..unsigned_int.member("mNumKeys", "Size of the #mKeys array. Must be 1, at least.")
    aiMeshKey_p.buffer("mKeys", "Key frames of the animation. May not be NULL.")
}
val aiMeshAnim_pp = aiMeshAnim.p.p

val aiAnimation = struct(ASSIMP_PACKAGE, "AIAnimation", nativeName = "aiAnimation") {
    documentation = """
        An animation consists of keyframe data for a number of nodes. For
        each node affected by the animation a separate series of data is given.
        """

    aiString.member("mName", "The name of the animation. If the modeling package this data was exported from does support only a single animation channel, this name is usually empty (length is zero).")
    ...
    aiNodeAnim_pp.member("mChannels", "The node animation channels. Each channel affects a single node. The array is mNumChannels in size.")
    AutoSize("mMeshChannels")..unsigned_int.member("mNumMeshChannels", "The number of mesh animation channels. Each channel affects a single mesh and defines vertex-based animation.")
    aiMeshAnim_pp.member("mMeshChannels", "The mesh animation channels. Each channel affects a single mesh. The array is mNumMeshChannels in size.")
}
val aiAnimation_pp = aiAnimation.p.p

val aiLightSourceType = "aiLightSourceType".enumType

val aiLight = struct(ASSIMP_PACKAGE, "AILight", nativeName = "aiLight") {
    documentation = """
    ...
    aiString.member("mName", "The name of the light source. There must be a node in the scenegraph with the same name. This node specifies the position of the light in the scene hierarchy and can be animated.")
    aiLightSourceType.member("mType", "The type of the light source. aiLightSource_UNDEFINED is not a valid value for this member.")
    aiVector3D.member("mPosition", "Position of the light source in space. Relative to the transformation of the node corresponding to the light. The position is undefined for directional lights.")
    aiVector3D.member("mDirection", "Direction of the light source in space. Relative to the transformation of the node corresponding to the light. The direction is undefined for point lights. The vector may be normalized, but it needn't.")
    aiVector3D.member("mUp", "Up direction of the light source in space. Relative to the transformation of the node corresponding to the light. The direction is undefined for point lights. The vector may be normalized, but it needn't.")
    float.member("mAttenuationConstant", "Constant light attenuation factor. The intensity of the light source at a given distance 'd' from the light's position is {@code Atten = 1/( att0 + att1 * d + att2 * d*d)}. This member corresponds to the att0 variable in the equation. Naturally undefined for directional lights.")
    float.member("mAttenuationLinear", "Linear light attenuation factor. The intensity of the light source at a given distance 'd' from the light's position is {@code Atten = 1/( att0 + att1 * d + att2 * d*d)}. This member corresponds to the att1 variable in the equation. Naturally undefined for directional lights.")
    float.member("mAttenuationQuadratic", "Quadratic light attenuation factor. The intensity of the light source at a given distance 'd' from the light's position is {@code Atten = 1/( att0 + att1 * d + att2 * d*d)}. This member corresponds to the att2 variable in the equation. Naturally undefined for directional lights.")
    aiColor3D.member("mColorDiffuse", "Diffuse color of the light source. The diffuse light color is multiplied with the diffuse material color to obtain the final color that contributes to the diffuse shading term.")
    aiColor3D.member("mColorSpecular", "Specular color of the light source. The specular light color is multiplied with the specular material color to obtain the final color that contributes to the specular shading term.")
    aiColor3D.member("mColorAmbient", "Ambient color of the light source. The ambient light color is multiplied with the ambient material color to obtain the final color that contributes to the ambient shading term. Most renderers will ignore this value it, is just a remaining of the fixed-function pipeline that is still supported by quite many file formats.")
    float.member("mAngleInnerCone", "Inner angle of a spot light's light cone. The spot light has maximum influence on objects inside this angle. The angle is given in radians. It is 2PI for point lights and undefined for directional lights.")
    float.member("mAngleOuterCone", "Outer angle of a spot light's light cone. The spot light does not affect objects outside this angle. The angle is given in radians. It is 2PI for point lights and undefined for directional lights. The outer angle must be greater than or equal to the inner angle. It is assumed that the application uses a smooth interpolation between the inner and the outer cone of the spot light.")
    aiVector2D.member("mSize", "Size of area light source.")
}
val aiLight_pp = aiLight.p.p

val aiCamera = struct(ASSIMP_PACKAGE, "AICamera", nativeName = "aiCamera") {
    documentation = """
    ...
    aiString.member("mName", "The name of the camera. There must be a node in the scenegraph with the same name. This node specifies the position of the camera in the scene hierarchy and can be animated.")
    ...
    aiVector3D.member("mUp", "'Up' - vector of the camera coordinate system relative to the coordinate space defined by the corresponding node. The 'right' vector of the camera coordinate system is the cross product of  the up and lookAt vectors. The default value is 0|1|0. The vector may be normalized, but it needn't.")
    aiVector3D.member("mLookAt", "'LookAt' - vector of the camera coordinate system relative to the coordinate space defined by the corresponding node. This is the viewing direction of the user. The default value is 0|0|1. The vector may be normalized, but it needn't.")
    float.member("mHorizontalFOV", "Half horizontal field of view angle, in radians. The field of view angle is the angle between the center line of the screen and the left or right border. The default value is 1/4PI.")
    float.member("mClipPlaneNear", "Distance of the near clipping plane from the camera. The value may not be 0.f (for arithmetic reasons to prevent a division through zero). The default value is 0.1f.")
    float.member("mClipPlaneFar", "Distance of the far clipping plane from the camera. The far clipping plane must, of course, be further away than the near clipping plane. The default value is 1000.f. The ratio between the near and the far plane should not be too large (between 1000-10000 should be ok) to avoid floating-point inaccuracies which could lead to z-fighting.")
    float.member("mAspect", "Screen aspect ratio. This is the ration between the width and the height of the screen. Typical values are 4/3, 1/2 or 1/1. This value is 0 if the aspect ratio is not defined in the source file. 0 is also the default value.")
}
val aiCamera_pp = aiCamera.p.p

val aiScene = struct(ASSIMP_PACKAGE, "AIScene", nativeName = "aiScene") {
    documentation = """
    ...
    unsigned_int.member("mFlags", "Any combination of the AI_SCENE_FLAGS_XXX flags. By default this value is 0, no flags are set. Most applications will want to reject all scenes with the AI_SCENE_FLAGS_INCOMPLETE bit set.")
    aiNode_p.member("mRootNode", "The root node of the hierarchy. There will always be at least the root node if the import was successful (and no special flags have been set). Presence of further nodes depends on the format and content of the imported file.")
    AutoSize("mMeshes")..unsigned_int.member("mNumMeshes", "The number of meshes in the scene.")
    aiMesh_pp.member("mMeshes", "The array of meshes. Use the indices given in the aiNode structure to access this array. The array is mNumMeshes in size. If the AI_SCENE_FLAGS_INCOMPLETE flag is not set there will always be at least ONE material.")
    AutoSize("mMaterials")..unsigned_int.member("mNumMaterials", "The number of materials in the scene.")
    aiMaterial_pp.member("mMaterials", "The array of materials. Use the index given in each aiMesh structure to access this array. The array is mNumMaterials in size. If the AI_SCENE_FLAGS_INCOMPLETE flag is not set there will always be at least ONE material.")
    AutoSize("mAnimations")..unsigned_int.member("mNumAnimations", "The number of animations in the scene.")
    aiAnimation_pp.member("mAnimations", "The array of animations. All animations imported from the given file are listed here. The array is mNumAnimations in size.")
    AutoSize("mTextures")..unsigned_int.member("mNumTextures", "The number of textures embedded into the file")
    aiTexture_pp.member("mTextures", "The array of embedded textures. Not many file formats embed their textures into the file. An example is Quake's MDL format (which is also used by some GameStudio versions)")
    AutoSize("mLights")..unsigned_int.member("mNumLights", "The number of light sources in the scene. Light sources are fully optional, in most cases this attribute will be 0")
    aiLight_pp.member("mLights", "The array of light sources. All light sources imported from the given file are listed here. The array is mNumLights in size.")
    AutoSize("mCameras")..unsigned_int.member("mNumCameras", "The number of cameras in the scene. Cameras are fully optional, in most cases this attribute will be 0")
    aiCamera_pp.member("mCameras", "The array of cameras. All cameras imported from the given file are listed here. The array is mNumCameras in size. The first camera in the array (if existing) is the default camera view into the scene.")
...
val aiTextureMapping_p = aiTextureMapping.p
