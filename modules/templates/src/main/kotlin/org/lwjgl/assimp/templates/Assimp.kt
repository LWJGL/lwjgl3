...
val Assimp = "Assimp".nativeClass(packageName = ASSIMP_PACKAGE, prefix = "ai", prefixConstant = "ai", binding = ASSIMP_BINDING) {
	...
	val ConfigurationKeys = StringConstant(
		"Configuration keys",
	...
	FloatConstant(
		"Default values for the Assimp##AI_CONFIG_PP_DB_THRESHOLD.",
...
aiAttachLogStream(&c);""")}
	...
	val SetImportPropertyInteger = void(
		...
		const..charASCII_p.IN("szName", "Name of the configuration property to be set.", ConfigurationKeys),
	...
	EnumConstant(
		...
		See the Assimp##_AI_MATKEY_MAPPING_BASE property for more details.
		""",

		"TextureMapping_UV".enum(
			"""
			The mapping coordinates are taken from an UV channel. The Assimp##_AI_MATKEY_UVWSRC_BASE key specifies from which UV channel the texture coordinates are
	...
	EnumConstant(
		...
		This corresponds to the Assimp##_AI_MATKEY_TEXFLAGS_BASE property.
		""",
	...
	EnumConstant(
		...
		"Process_CalcTangentSpace".enum(
			...
			such as normal mapping  applied to the meshes. There's an importer property, Assimp##AI_CONFIG_PP_CT_MAX_SMOOTHING_ANGLE, which allows you to
		...
		"Process_RemoveComponent".enum(
			...
			The  components to be removed are specified in a separate importer property, Assimp##AI_CONFIG_PP_RVC_FLAGS. This is quite useful if you don't need
		...
		"Process_GenSmoothNormals".enum(
			...
			This flag may not be specified together with #Process_GenNormals. There's a importer property, Assimp##AI_CONFIG_PP_GSN_MAX_SMOOTHING_ANGLE which
		...
		"Process_SplitLargeMeshes".enum(
			...
			The split limits can (and should!) be set through the Assimp##AI_CONFIG_PP_SLM_VERTEX_LIMIT and Assimp##AI_CONFIG_PP_SLM_TRIANGLE_LIMIT importer
			properties. The default values are Assimp##AI_SLM_DEFAULT_MAX_VERTICES and Assimp##AI_SLM_DEFAULT_MAX_TRIANGLES.
		...
		"Process_PreTransformVertices".enum(
			...
			${note(
				"""
				The Assimp##AI_CONFIG_PP_PTV_NORMALIZE configuration property can be set to normalize the scene's spatial dimension to the {@code -1...1}
				range.
				""")}
			""",
			0x100
		...
		"Process_LimitBoneWeights".enum(
			...
			are renormalized so that the weights still sum up to 1. The default bone weight limit is 4 (defined as Assimp##AI_LMW_MAX_WEIGHTS in config.h), but
			you can use the Assimp##AI_CONFIG_PP_LBW_MAX_WEIGHTS importer property to supply your own limit to the post processing step.
		...
		"Process_ValidateDataStructure".enum(
			...
			${ul(
				...
				structure is still safe. Warning details are written to the log file, Assimp##AI_SCENE_FLAGS_VALIDATION_WARNING is set in
				##AIScene{@code ::mFlags}.
				"""
			)}
		...
		"Process_ImproveCacheLocality".enum(
			...
			If you intend to render huge models in hardware, this step might be of interest to you. The Assimp##AI_CONFIG_PP_ICL_PTCACHE_SIZE importer property
		...
		"Process_RemoveRedundantMaterials".enum(
			...
			take a look at the Assimp##AI_CONFIG_PP_RRM_EXCLUDE_LIST importer property.
			""",
			0x1000
		),
		...
		"Process_SortByPType".enum(
			...
			Assimp##AI_CONFIG_PP_SBP_REMOVE importer property to specify which primitive types you need. This can be used to easily exclude lines and points,
		...
		"Process_FindDegenerates".enum(
			...
			${ol(
				...
				${ul(
					...
					"""
					Set the Assimp##AI_CONFIG_PP_FD_REMOVE importer property to 1. This will cause the step to remove degenerate triangles from the import as
				...
				${ul(
					...
					"""
					Set the Assimp##AI_CONFIG_PP_SBP_REMOVE importer property to ${code("#PrimitiveType_POINT | #PrimitiveType_LINE")} to cause
		...
		"Process_FindInvalidData".enum(
			...
			The Assimp##AI_CONFIG_PP_FID_ANIM_ACCURACY config property decides the accuracy of the check for duplicate animation tracks.
			""",
			0x20000
		),
		...
		"Process_GenUVCoords".enum(
			...
			${note(
				"""
				If this step is not requested, you'll need to process the Assimp##_AI_MATKEY_MAPPING_BASE material property in order to display all assets
				properly.
				""")}
			""",
			0x40000
		...
		"Process_TransformUVCoords".enum(
			...
			UV transformations are specified per-texture - see the Assimp##_AI_MATKEY_UVTRANSFORM_BASE material key for more information. This step processes
		...
		"Process_OptimizeGraph".enum(
			...
			Assimp##AI_CONFIG_PP_OG_EXCLUDE_LIST importer property to specify a list of node names you want to be kept. Nodes matching one of the names in this
...
x1""")}
		...
		"Process_Debone".enum(
			...
			Use Assimp##AI_CONFIG_PP_DB_THRESHOLD to control this. Use Assimp##AI_CONFIG_PP_DB_ALL_OR_NONE if you want bones removed if and only if all bones
