...
val GLFWVulkan = dependsOn(Binding.VULKAN) {
	"GLFWVulkan".nativeClass(packageName = GLFW_PACKAGE, prefix = "GLFW", binding = GLFW_BINDING_DELEGATE) {
		...
		intb(
			...
			created with the required extensions, this function returns #FALSE and generates a #API_UNAVAILABLE error. Call #VulkanSupported() to check
		...
		VkResult(
			...
			#API_UNAVAILABLE error. Call #VulkanSupported() to check whether the Vulkan loader was found.
			...
			enabled, this function returns VK10#VK_ERROR_EXTENSION_NOT_PRESENT and generates a #API_UNAVAILABLE error. Call
