...
val GLFWVulkan = dependsOn(Binding.VULKAN) {
	"GLFWVulkan".nativeClass(packageName = GLFW_PACKAGE, prefix = "GLFW", binding = GLFW_BINDING_DELEGATE) {
		...
		GLFWvkproc(
			...
			If Vulkan is not available on the machine, this function returns $NULL and generates a GLFW#API_UNAVAILABLE error. Call GLFW#VulkanSupported() to
			check whether Vulkan is available.
		...
		int(
			...
			created with the required extensions, this function returns GLFW#FALSE and generates a GLFW#API_UNAVAILABLE error. Call GLFW#VulkanSupported() to
			check whether Vulkan is available and GLFW#GetRequiredInstanceExtensions() to check what instance extensions are required.
		...
		VkResult(
			...
			GLFW#API_UNAVAILABLE error. Call GLFW#VulkanSupported() to check whether the Vulkan loader was found.
			...
			GLFW does not destroy it for you. Call VK10##vkDestroySurfaceKHR() to destroy the surface.
			...
			GLFW#VulkanSupported() and GLFW#GetRequiredInstanceExtensions() should eliminate almost all occurrences of these errors.
			...
			Check(1)..VkSurfaceKHR_p.OUT("surface", "where to store the handle of the surface. This is set to {@code VK_NULL_HANDLE} if an error occurred."),
