...
public class CXUnsavedFile extends Struct implements NativeResource {
    ...
    /**
     * Creates a {@link CXUnsavedFile} instance at the current position of the specified {@link ByteBuffer} container. Changes to the buffer's content will be
    ...
    /** Returns a new {@link CXUnsavedFile} instance allocated with {@link MemoryUtil#memAlloc memAlloc}. The instance must be explicitly freed. */
    public static CXUnsavedFile malloc() {
        return wrap(CXUnsavedFile.class, nmemAllocChecked(SIZEOF));
    }

    /** Returns a new {@link CXUnsavedFile} instance allocated with {@link MemoryUtil#memCalloc memCalloc}. The instance must be explicitly freed. */
    public static CXUnsavedFile calloc() {
        return wrap(CXUnsavedFile.class, nmemCallocChecked(1, SIZEOF));
    }

    /** Returns a new {@link CXUnsavedFile} instance allocated with {@link BufferUtils}. */
    public static CXUnsavedFile create() {
        ByteBuffer container = BufferUtils.createByteBuffer(SIZEOF);
        return wrap(CXUnsavedFile.class, memAddress(container), container);
    }

    /** Returns a new {@link CXUnsavedFile} instance for the specified memory address. */
    ...
    /** Returns a new {@link CXUnsavedFile} instance allocated on the thread-local {@link MemoryStack}. */
    public static CXUnsavedFile mallocStack() {
        return mallocStack(stackGet());
    }

    /** Returns a new {@link CXUnsavedFile} instance allocated on the thread-local {@link MemoryStack} and initializes all its bits to zero. */
    public static CXUnsavedFile callocStack() {
        return callocStack(stackGet());
    }

    /**
     * Returns a new {@link CXUnsavedFile} instance allocated on the specified {@link MemoryStack}.
     *
    ...
    /**
     * Returns a new {@link CXUnsavedFile} instance allocated on the specified {@link MemoryStack} and initializes all its bits to zero.
     *
     * @param stack the stack from which to allocate
     */
    ...
    public static class Buffer extends StructBuffer<CXUnsavedFile, Buffer> implements NativeResource {
        ...
        /**
         * Creates a new {@link CXUnsavedFile.Buffer} instance backed by the specified container.
         *
