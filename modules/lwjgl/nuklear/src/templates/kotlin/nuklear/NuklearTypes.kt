...
val nk_convert_config = struct(Module.NUKLEAR, "NkConvertConfig", nativeName = "struct nk_convert_config") {
    ...
    unsigned_int("circle_segment_count")
    unsigned_int("arc_segment_count")
    unsigned_int("curve_segment_count")
    nk_draw_null_texture("tex_null")
    Unsafe..nk_draw_vertex_layout_element.p("vertex_layout")
    nk_size("vertex_size")
...
val nk_user_font = struct(Module.NUKLEAR, "NkUserFont", nativeName = "struct nk_user_font") {
    nk_handle("userdata")
    float("height")
    nullable..nk_text_width_f("width")
    nullable..nk_query_font_glyph_f("query")
    nk_handle("texture")
}
...
/* private:
    ...
    (nullable.."nk_window".opaque.p("begin")).private()
    (nullable.."nk_window".opaque.p("end")).private()
    (nullable.."nk_window".opaque.p("active")).private()
    (nullable.."nk_window".opaque.p("current")).private()
...
val nk_baked_font = struct(Module.NUKLEAR, "NkBakedFont", nativeName = "struct nk_baked_font") {
    ...
    nk_rune("glyph_count")
    nullable..nk_rune.const.p("ranges")
}
...
val nk_font_config = struct(Module.NUKLEAR, "NkFontConfig", nativeName = "struct nk_font_config") {
    nullable.._nk_font_config.p("next")
    nullable..void.p("ttf_blob")
    AutoSize("ttf_blob")..nk_size("ttf_size")
    unsigned_charb("ttf_data_owned_by_atlas")
    unsigned_charb("merge_mode")
    unsigned_charb("pixel_snap")
    unsigned_charb("oversample_v")
    unsigned_charb("oversample_h")
    unsigned_char("padding")[3]
    float("size")
    nk_font_coord_type("coord_type")
    nk_vec2("spacing")
    nullable..nk_rune.const.p("range")
    nullable..nk_baked_font.p("font")
