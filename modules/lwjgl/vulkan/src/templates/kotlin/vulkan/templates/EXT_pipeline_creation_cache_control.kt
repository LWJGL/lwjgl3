...
val EXT_pipeline_creation_cache_control = "EXTPipelineCreationCacheControl".nativeClassVK("EXT_pipeline_creation_cache_control", type = "device", postfix = "EXT") {
    documentation =
        ...
        Applications can prevent unexpected compilation by setting #PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT_EXT on stext:Vk*PipelineCreateInfo{@code ::flags}. When set, an ICD must not attempt pipeline or shader compilation to create the pipeline object. In such a case, if the implementation fails to create a pipeline without compilation, the implementation <b>must</b> return the result #PIPELINE_COMPILE_REQUIRED_EXT and return #NULL_HANDLE for the pipeline.
        ...
        Hidden locks also add to the unpredictability of the cost of pipeline creation. The most common case of locks inside the stext:vkCreate*Pipelines is internal synchronization of the {@code VkPipelineCache} object. #PIPELINE_CACHE_CREATE_EXTERNALLY_SYNCHRONIZED_BIT_EXT can be set when calling #CreatePipelineCache() to state the cache is <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#fundamentals-threadingbehavior">externally synchronized</a>.
        ...
        <dl>
            ...
            <dt><b>Contact</b></dt>
            <dd><ul>
                <li>Gregory Grebe <a href="https://github.com/KhronosGroup/Vulkan-Docs/issues/new?body=[VK_EXT_pipeline_creation_cache_control]%20@grgrebe_amd%250A*Here%20describe%20the%20issue%20or%20question%20you%20have%20about%20the%20VK_EXT_pipeline_creation_cache_control%20extension*">grgrebe_amd</a></li>
            </ul></dd>
        </dl>
