...
val INTEL_performance_query = "INTELPerformanceQuery".nativeClassVK("INTEL_performance_query", type = "device", postfix = "INTEL") {
    documentation =
        ...
        Performance analysis tools such as <a href="https://www.intel.com/content/www/us/en/developer/tools/graphics-performance-analyzers/overview.html">Graphics Performance Analyzers</a> make use of this extension and the metrics-discovery library to present the data in a human readable way.
...
￿    VkPerformanceConfigurationINTEL             configuration);</code></pre>
        ...
        <h5>Valid Usage</h5>
        <ul>
            <li>{@code configuration} <b>must</b> not be released before all command buffers submitted while the configuration was set are in <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#commandbuffers-lifecycle">pending state</a></li>
        </ul>

        <h5>Valid Usage (Implicit)</h5>
        <ul>
            <li>{@code device} <b>must</b> be a valid {@code VkDevice} handle</li>
            <li>If {@code configuration} is not #NULL_HANDLE, {@code configuration} <b>must</b> be a valid {@code VkPerformanceConfigurationINTEL} handle</li>
