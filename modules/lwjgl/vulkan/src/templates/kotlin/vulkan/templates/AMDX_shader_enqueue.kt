...
val AMDX_shader_enqueue = "AMDXShaderEnqueue".nativeClassVK("AMDX_shader_enqueue", type = "device", postfix = "AMDX") {
    documentation =
        ...
        <dl>
            ...
            <dd>{@link KHRSynchronization2 VK_KHR_synchronization2} and {@link KHRSpirv14 VK_KHR_spirv_1_4} and {@link EXTExtendedDynamicState VK_EXT_extended_dynamic_state} or <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#versions-1.3">Version 1.3</a> and {@link KHRMaintenance5 VK_KHR_maintenance5} and {@link KHRPipelineLibrary VK_KHR_pipeline_library}
...
￿    VkPipeline*                                 pPipelines);</code></pre>
        ...
        <h5>Valid Usage</h5>
        <ul>
            <li>The <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#features-shaderEnqueue">{@code shaderEnqueue}</a> feature <b>must</b> be enabled</li>
...
￿    VkExecutionGraphPipelineScratchSizeAMDX*    pSizeInfo);</code></pre>
        ...
        VkDevice("device", "the logical device that {@code executionGraph} was created on."),
...
￿    VkDeviceSize                                scratchSize);</code></pre>
        ...
        <h5>Description</h5>
        This command <b>must</b> be called before using {@code scratch} to dispatch the bound execution graph pipeline.

        Execution of this command <b>may</b> modify any memory locations in the range [{@code scratch},{@code scratch} + {@code scratchSize}). Accesses to this memory range are performed in the #PIPELINE_STAGE_2_COMPUTE_SHADER_BIT pipeline stage with the #ACCESS_2_SHADER_STORAGE_READ_BIT and #ACCESS_2_SHADER_STORAGE_WRITE_BIT access flags.

        If any portion of {@code scratch} is modified by any command other than #CmdDispatchGraphAMDX(), #CmdDispatchGraphIndirectAMDX(), #CmdDispatchGraphIndirectCountAMDX(), or #CmdInitializeGraphScratchMemoryAMDX() with the same execution graph, it <b>must</b> be reinitialized for the execution graph again before dispatching against it.

        <h5>Valid Usage</h5>
        <ul>
            <li>{@code scratch} <b>must</b> be the device address of an allocated memory range at least as large as {@code scratchSize}</li>
            <li>{@code scratchSize} <b>must</b> be greater than or equal to ##VkExecutionGraphPipelineScratchSizeAMDX{@code ::minSize} returned by #GetExecutionGraphPipelineScratchSizeAMDX() for the bound execution graph pipeline</li>
        ...
        VkPipeline("executionGraph", "the execution graph pipeline to initialize the scratch memory for."),
        VkDeviceAddress("scratch", "the address of scratch memory to be initialized."),
        VkDeviceSize("scratchSize", "a range in bytes of scratch memory to be initialized.")
    )
...
￿    const VkDispatchGraphCountInfoAMDX*         pCountInfo);</code></pre>
        ...
        <h5>Description</h5>
        When this command is executed, the nodes specified in {@code pCountInfo} are executed. Nodes executed as part of this command are not implicitly synchronized in any way against each other once they are dispatched. There are no rasterization order guarantees between separately dispatched graphics nodes, though individual primitives within a single dispatch do adhere to rasterization order. Draw calls executed before or after the execution graph also execute relative to each graphics node with respect to rasterization order.
        ...
        This command <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#executiongraphs-meshnodes-statecapture">captures command buffer state</a> for mesh nodes similarly to draw commands.
        ...
        <ul>
            ...
            <li>The device memory range [{@code scratch},{@code scratch} {@code scratchSize}] <b>must</b> have been initialized with #CmdInitializeGraphScratchMemoryAMDX() using the bound execution graph pipeline, and not modified after that by anything other than another execution graph dispatch command</li>
            <li>Execution of this command <b>must</b> not cause a node to be dispatched with a larger number of workgroups than that specified by either a {@code MaxNumWorkgroupsAMDX} decoration in the dispatched node or <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-maxComputeWorkGroupCount">{@code maxComputeWorkGroupCount}</a></li>
            <li>Execution of this command <b>must</b> not cause any shader to initialize more than <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-maxExecutionGraphShaderPayloadCount">{@code maxExecutionGraphShaderPayloadCount}</a> output payloads</li>
            ...
            <li>If the bound execution graph pipeline includes draw nodes, this command <b>must</b> be called within a render pass instance that is compatible with the graphics pipeline used to create each of those nodes</li>
            ...
            <li>For each ##VkDispatchGraphInfoAMDX structure in {@code pCountInfo→infos}, {@code nodeIndex} <b>must</b> be a valid node index in the bound execution graph pipeline, as returned by #GetExecutionGraphPipelineNodeIndexAMDX()</li>
        ...
        VkCommandBuffer("commandBuffer", "the command buffer into which the command will be recorded."),
        VkDeviceAddress("scratch", "the address of scratch memory to be used."),
        VkDeviceSize("scratchSize", "a range in bytes of scratch memory to be used."),
...
￿    const VkDispatchGraphCountInfoAMDX*         pCountInfo);</code></pre>
        ...
        <ul>
            ...
            <li>{@code pCountInfo→infos} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
            ...
            <li>For each ##VkDispatchGraphInfoAMDX structure in {@code pCountInfo→infos}, {@code payloads} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
            <li>For each ##VkDispatchGraphInfoAMDX structure in {@code pCountInfo→infos}, {@code nodeIndex} <b>must</b> be a valid node index in the bound execution graph pipeline, as returned by #GetExecutionGraphPipelineNodeIndexAMDX() when this command is executed on the device</li>
...
￿    VkDeviceAddress                             countInfo);</code></pre>
        ...
        <ul>
            ...
            <li>{@code countInfo} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
            ...
            <li>{@code countInfo→infos} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
            <li>Device memory locations at indexes in the range [{@code infos}, {@code infos} + ({@code count}*{@code stride})), at a granularity of {@code stride} <b>must</b> contain valid ##VkDispatchGraphInfoAMDX structures in the first 24 bytes when this command is executed on the device</li>
            ...
            <li>For each ##VkDispatchGraphInfoAMDX structure in {@code countInfo→infos}, {@code payloads} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
            <li>For each ##VkDispatchGraphInfoAMDX structure in {@code countInfo→infos}, {@code nodeIndex} <b>must</b> be a valid node index in the bound execution graph pipeline, as returned by #GetExecutionGraphPipelineNodeIndexAMDX() when this command is executed on the device</li>
