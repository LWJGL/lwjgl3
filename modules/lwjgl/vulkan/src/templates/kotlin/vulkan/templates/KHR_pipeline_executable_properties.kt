...
￿    VkPipelineExecutablePropertiesKHR*          pProperties);</code></pre>
        ...
        <h5>Description</h5>
        If {@code pProperties} is {@code NULL}, then the number of executables associated with the pipeline is returned in {@code pExecutableCount}. Otherwise, {@code pExecutableCount} <b>must</b> point to a variable set by the user to the number of elements in the {@code pProperties} array, and on return the variable is overwritten with the number of structures actually written to {@code pProperties}. If {@code pExecutableCount} is less than the number of executables associated with the pipeline, at most {@code pExecutableCount} structures will be written and {@code vkGetPipelineExecutablePropertiesKHR} will return #INCOMPLETE.

        <h5>Valid Usage</h5>
        <ul>
            <li><a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#features-pipelineExecutableProperties">{@code pipelineExecutableProperties}</a> <b>must</b> be enabled.</li>
...
￿    VkPipelineExecutableStatisticKHR*           pStatistics);</code></pre>
        ...
        <ul>
            ...
            <li>{@code pipeline} member of {@code pExecutableInfo} <b>must</b> have been created with #PIPELINE_CREATE_CAPTURE_STATISTICS_BIT_KHR set in the {@code flags} field of ##VkGraphicsPipelineCreateInfo or ##VkComputePipelineCreateInfo.</li>
        </ul>
...
￿    VkPipelineExecutableInternalRepresentationKHR* pInternalRepresentations);</code></pre>
        ...
        <h5>Description</h5>
        If {@code pInternalRepresentations} is {@code NULL}, then the number of internal representations associated with the pipeline executable is returned in {@code pInternalRepresentationCount}. Otherwise, {@code pInternalRepresentationCount} <b>must</b> point to a variable set by the user to the number of elements in the {@code pInternalRepresentations} array, and on return the variable is overwritten with the number of structures actually written to {@code pInternalRepresentations}. If {@code pInternalRepresentationCount} is less than the number of internal representations associated with the pipeline executable, at most {@code pInternalRepresentationCount} structures will be written and {@code vkGetPipelineExecutableInternalRepresentationsKHR} will return #INCOMPLETE.

        While the details of the internal representations remain implementation dependent, the implementation <b>should</b> order the internal representations in the order in which they occur in the compile pipeline with the final shader assembly (if any) last.

        <h5>Valid Usage</h5>
        <ul>
            <li><a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#features-pipelineExecutableProperties">{@code pipelineExecutableProperties}</a> <b>must</b> be enabled.</li>
            <li>{@code pipeline} member of {@code pExecutableInfo} <b>must</b> have been created with {@code device}.</li>
            <li>{@code pipeline} member of {@code pExecutableInfo} <b>must</b> have been created with #PIPELINE_CREATE_CAPTURE_INTERNAL_REPRESENTATIONS_BIT_KHR set in the {@code flags} field of ##VkGraphicsPipelineCreateInfo or ##VkComputePipelineCreateInfo.</li>
        </ul>
