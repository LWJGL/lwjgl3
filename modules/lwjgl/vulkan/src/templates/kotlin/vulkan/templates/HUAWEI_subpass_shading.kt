...
val HUAWEI_subpass_shading = "HUAWEISubpassShading".nativeClassVK("HUAWEI_subpass_shading", type = "device", postfix = "HUAWEI") {
    documentation =
        ...
        The {@code GlobalInvocationId.xy} of a subpass shading pipeline is equal to the {@code FragCoord.xy} of a graphic pipeline in the same render pass subtracted the <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#VkRect2D">{@code offset}</a> of the ##VkRenderPassBeginInfo{@code ::renderArea}. {@code GlobalInvocationId.z} is mapped to the Layer if {@link EXTShaderViewportIndexLayer VK_EXT_shader_viewport_index_layer} is supported. The {@code GlobalInvocationId.xy} is equal to the index of the local workgroup multiplied by the size of the local workgroup plus the {@code LocalInvocationId} and the <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#VkRect2D">{@code offset}</a> of the ##VkRenderPassBeginInfo{@code ::renderArea}.
...
￿vkCreateComputePipelines(device, pipelineCache, 1, &amp;subpassShadingComputePipelineCreateInfo, NULL, &amp;pipeline);</code></pre>
        ...
        <dl>
            ...
            <dd><ul>
                <li>This extension requires <a href="https://htmlpreview.github.io/?https://github.com/KhronosGroup/SPIRV-Registry/blob/master/extensions/HUAWEI/SPV_HUAWEI_subpass_shading.html">{@code SPV_HUAWEI_subpass_shading}</a>.</li>
                <li>This extension provides API support for <a href="https://github.com/KhronosGroup/GLSL/blob/master/extensions/huawei/GLSL_HUAWEI_subpass_shading.txt">{@code GL_HUAWEI_subpass_shading}</a>.</li>
            </ul></dd>
