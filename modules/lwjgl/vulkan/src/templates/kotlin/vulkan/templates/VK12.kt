...
￿    uint32_t                                    stride);</code></pre>
        ...
        <ul>
            ...
            <li>If the bound graphics pipeline was created with the #DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE dynamic state enabled, but without the #DYNAMIC_STATE_VERTEX_INPUT_EXT dynamic state enabled, then #CmdBindVertexBuffers2() <b>must</b> have been called and not subsequently <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#dynamic-state-lifetime">invalidated</a> in the current command buffer prior to this draw command, and the {@code pStrides} parameter of #CmdBindVertexBuffers2() <b>must</b> not be {@code NULL}</li>
            ...
            <li>If there is a shader object bound to the #SHADER_STAGE_TESSELLATION_CONTROL_BIT stage or the bound graphics pipeline state was created with the #DYNAMIC_STATE_PATCH_CONTROL_POINTS_EXT dynamic state enabled, and the <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#dynamic-state-current-value">current value</a> of {@code primitiveTopology} is #PRIMITIVE_TOPOLOGY_PATCH_LIST, then #CmdSetPatchControlPointsEXT() <b>must</b> have been called and not subsequently <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html\#dynamic-state-lifetime">invalidated</a> in the current command buffer prior to this drawing command</li>
