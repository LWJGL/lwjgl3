...
val KHR_video_decode_queue = "KHRVideoDecodeQueue".nativeClassVK("KHR_video_decode_queue", type = "device", postfix = "KHR") {
    ...
    EnumConstant(
        ...
        <h5>Description</h5>
        <ul>
            <li>#VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_COINCIDE_BIT_KHR indicates support for using the same video picture resource as the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#reconstructed-picture">reconstructed picture</a> and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-output-picture">decode output picture</a> in a video decode operation.</li>
            <li>
                #VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR indicates support for using distinct video picture resources as the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#reconstructed-picture">reconstructed picture</a> and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-output-picture">decode output picture</a> in a video decode operation.
                <div style="margin-left: 26px; border-left: 1px solid gray; padding-left: 14px;"><h5>Note</h5>
                Some video profiles allow using distinct video picture resources as the reconstructed picture and decode output picture in specific video decode operations even when the video decode profile does not support #VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR. Even if the implementation only reports coincide, the decode output picture for <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-av1-film-grain">film grain</a> enabled frames must be a different video picture resource from the reconstructed picture because film grain is applied outside of the coding loop.
                </div>
            </li>
        </ul>
...
￿    const VkVideoDecodeInfoKHR*                 pDecodeInfo);</code></pre>
        ...
        <h5>Description</h5>
        Each call issues one or more video decode operations. The implicit parameter {@code opCount} corresponds to the number of video decode operations issued by the command. After calling this command, the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#queries-operation-active-query-index">active query index</a> of each <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#queries-operation-active">active</a> query is incremented by {@code opCount}.

        Currently each call to this command results in the issue of a single video decode operation.

        If the bound video session was created with #VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR and the {@code pNext} chain of {@code pDecodeInfo} includes a ##VkVideoInlineQueryInfoKHR structure with its {@code queryPool} member specifying a valid {@code VkQueryPool} handle, then this command will execute a query for each video decode operation issued by it.
        ...
        Specifying a valid ##VkVideoReferenceSlotInfoKHR structure in {@code pDecodeInfo→pSetupReferenceSlot} is always required, unless the video session was created with ##VkVideoSessionCreateInfoKHR{@code ::maxDpbSlot} equal to zero. However, the DPB slot identified by {@code pDecodeInfo→pSetupReferenceSlot→slotIndex} is only <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#dpb-slot-states">activated</a> with the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#reconstructed-picture">reconstructed picture</a> specified in {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} if reference picture setup is requested according to the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-codec-specific-semantics">codec-specific semantics</a>.

        If reconstructed picture information is specified, and {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} refers to a <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resources">video picture resource</a> different than that of the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-output-picture">decode output picture</a>, but reference picture setup is not requested, the contents of the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resources">video picture resource</a> corresponding to the reconstructed picture will be undefined after the video decode operation.
        ...
        <ul>
            <li>
                ...
                <ul>
                    <li>
                        If the bound video session was created with an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-profile">H.264 decode profile</a>, then let {@code activeReferencePictureCount} be the value of {@code pDecodeInfo→referenceSlotCount} plus the number of elements of the {@code pDecodeInfo→pReferenceSlots} array that have a ##VkVideoDecodeH264DpbSlotInfoKHR structure included in their {@code pNext} chain with both {@code pStdReferenceInfo→flags.top_field_flag} and {@code pStdReferenceInfo→flags.bottom_field_flag} set.
                        <div style="margin-left: 26px; border-left: 1px solid gray; padding-left: 14px;"><h5>Note</h5>
                        This means that the elements of {@code pDecodeInfo→pReferenceSlots} that include both a top and bottom field reference are counted as two separate active reference pictures, as described in the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-active-reference-picture-info">active reference picture list construction rules for H.264 decode operations</a>.
                        </div>
                    </li>
                </ul>
            </li>
            ...
            <li>
                ...
                <ul>
                    <li>If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then {@code dpbFrameUseCount[i]} is incremented by one, where {@code i} equals {@code pDecodeInfo→pSetupReferenceSlot→slotIndex}. If the bound video session object was created with an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-profile">H.264 decode profile</a>, then {@code dpbFrameUseCount[i]} is decremented by one if either {@code pStdReferenceInfo→flags.top_field_flag} or {@code pStdReferenceInfo→flags.bottom_field_flag} is set in the ##VkVideoDecodeH264DpbSlotInfoKHR structure in the {@code pDecodeInfo→pSetupReferenceSlot→pNext} chain.</li>
                    <li>For each element of {@code pDecodeInfo→pReferenceSlots}, {@code dpbFrameUseCount[i]} is incremented by one, where {@code i} equals the {@code slotIndex} member of the corresponding element. If the bound video session object was created with an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-profile">H.264 decode profile</a>, then {@code dpbFrameUseCount[i]} is decremented by one if either {@code pStdReferenceInfo→flags.top_field_flag} or {@code pStdReferenceInfo→flags.bottom_field_flag} is set in the ##VkVideoDecodeH264DpbSlotInfoKHR structure in the {@code pNext} chain of the corresponding element of {@code pDecodeInfo→pReferenceSlots}.</li>
                </ul>
            </li>
            <li>
                ...
                <ul>
                    <li>
                        If the bound video session object was created with an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-profile">H.264 decode profile</a> and {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then perform the following:
                        <ul>
                    ...
                    <li>
                        If the bound video session object was created with an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-profile">H.264 decode profile</a>, then perform the following for each element of {@code pDecodeInfo→pReferenceSlots}:
                        <ul>
        ...
        <ul>
            ...
            <li>If the bound video session was created with #VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR, and the {@code pNext} chain of {@code pDecodeInfo} includes a ##VkVideoInlineQueryInfoKHR structure with its {@code queryPool} member specifying a valid {@code VkQueryPool} handle, then ##VkVideoInlineQueryInfoKHR::queryCount <b>must</b> equal {@code opCount}</li>
            ...
            <li>If the bound video session was created with #VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR, then the {@code queryType} used to create the {@code queryPool} specified in the ##VkVideoInlineQueryInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> be #QUERY_TYPE_RESULT_STATUS_ONLY_KHR</li>
            <li>If the bound video session was created with #VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR, then the {@code queryPool} specified in the ##VkVideoInlineQueryInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> have been created with a ##VkVideoProfileInfoKHR structure included in the {@code pNext} chain of ##VkQueryPoolCreateInfo identical to the one specified in ##VkVideoSessionCreateInfoKHR{@code ::pVideoProfile} the bound video session was created with</li>
            <li>If the bound video session was created with #VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR, and the {@code queryType} used to create the {@code queryPool} specified in the ##VkVideoInlineQueryInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} is #QUERY_TYPE_RESULT_STATUS_ONLY_KHR, then the {@code VkCommandPool} that {@code commandBuffer} was allocated from <b>must</b> have been created with a queue family index that supports <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#queries-result-status-only">result status queries</a>, as indicated by ##VkQueueFamilyQueryResultStatusPropertiesKHR{@code ::queryResultStatusSupport}</li>
            <li>{@code pDecodeInfo→srcBuffer} <b>must</b> be <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-profile-compatibility">compatible</a> with the video profile the bound video session was created with</li>
            <li>If {@code commandBuffer} is an unprotected command buffer and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-protectedNoFault">{@code protectedNoFault}</a> is not supported, then {@code pDecodeInfo→srcBuffer} <b>must</b> not be a protected buffer</li>
            <li>If {@code commandBuffer} is a protected command buffer and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#limits-protectedNoFault">{@code protectedNoFault}</a> is not supported, then {@code pDecodeInfo→srcBuffer} <b>must</b> be a protected buffer</li>
            ...
            <li>If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL} and ##VkVideoDecodeCapabilitiesKHR{@code ::flags} does not include #VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_COINCIDE_BIT_KHR, as returned by #GetPhysicalDeviceVideoCapabilitiesKHR() for the video profile the bound video session was created with, then the video picture resources specified by {@code pDecodeInfo→dstPictureResource} and {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <b>must</b> not <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">match</a></li>
            <li>
                If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL} and none of the following is true:
                <ul>
                    <li>##VkVideoDecodeCapabilitiesKHR{@code ::flags} includes #VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR, as returned by #GetPhysicalDeviceVideoCapabilitiesKHR() for the video profile the bound video session was created with</li>
                    <li>the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR and ##VkVideoDecodeAV1ProfileInfoKHR{@code ::filmGrainSupport} set to #TRUE, and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-av1-film-grain">film grain</a> is enabled for the decoded picture</li>
                </ul>

                then the video picture resources specified by {@code pDecodeInfo→dstPictureResource} and {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <b>must</b> <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">match</a>
            </li>
            <li>{@code pDecodeInfo→dstPictureResource.imageViewBinding} <b>must</b> be <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-profile-compatibility">compatible</a> with the video profile the bound video session was created with</li>
            ...
            <li>{@code pDecodeInfo→pSetupReferenceSlot} <b>must</b> not be {@code NULL} unless the bound video session was created with ##VkVideoSessionCreateInfoKHR{@code ::maxDpbSlots} equal to zero</li>
            ...
            <li>If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <b>must</b> <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">match</a> one of the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#bound-reference-picture-resources">bound reference picture resource</a></li>
            ...
            <li>The {@code pPictureResource} member of each element of {@code pDecodeInfo→pReferenceSlots} <b>must</b> <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">match</a> one of the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#bound-reference-picture-resources">bound reference picture resource</a> associated with the DPB slot index specified in the {@code slotIndex} member of that element</li>
            <li>Each video picture resource corresponding to the {@code pPictureResource} member specified in the elements of {@code pDecodeInfo→pReferenceSlots} <b>must</b> be <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-uniqueness">unique</a> within {@code pDecodeInfo→pReferenceSlots}</li>
            ...
            <li>If {@code pDecodeInfo→pSetupReferenceSlot} is {@code NULL} or {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} does not <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-image-subresource-reference">refer</a> to the same image subresource as {@code pDecodeInfo→dstPictureResource}, then the image subresource <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-image-subresource-reference">referred</a> to by {@code pDecodeInfo→dstPictureResource} <b>must</b> be in the #IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR layout at the time the video decode operation is executed on the device</li>
            <li>If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL} and {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-image-subresource-reference">refers</a> to the same image subresource as {@code pDecodeInfo→dstPictureResource}, then the image subresource <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-image-subresource-reference">referred</a> to by {@code pDecodeInfo→dstPictureResource} <b>must</b> be in the #IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR layout at the time the video decode operation is executed on the device</li>
            <li>If {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then the image subresource <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-image-subresource-reference">referred</a> to by {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <b>must</b> be in the #IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR layout at the time the video decode operation is executed on the device</li>
            ...
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR but was not created with <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-interlaced-support">interlaced frame support</a>, then the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-output-picture-info">decode output picture</a> <b>must</b> represent a frame</li>
            ...
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR but was not created with <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-interlaced-support">interlaced frame support</a>, and {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-reconstructed-picture-info">reconstructed picture</a> <b>must</b> represent a frame</li>
            ...
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR but was not created with <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-interlaced-support">interlaced frame support</a>, then each <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-active-reference-picture-info">active reference picture</a> corresponding to the elements of {@code pDecodeInfo→pReferenceSlots} <b>must</b> represent a frame</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR, {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, and the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-output-picture-info">decode output picture</a> represents a frame, then the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-reconstructed-picture-info">reconstructed picture</a> <b>must</b> also represent a frame</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR, {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, and the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-output-picture-info">decode output picture</a> represents a top field, then the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-reconstructed-picture-info">reconstructed picture</a> <b>must</b> also represent a top field</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR, {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, and the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-output-picture-info">decode output picture</a> represents a bottom field, then the <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-reconstructed-picture-info">reconstructed picture</a> <b>must</b> also represent a bottom field</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR and an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-active-reference-picture-info">active reference picture</a> corresponding to any element of {@code pDecodeInfo→pReferenceSlots} represents a frame, then the DPB slot index of the bound video session specified by the {@code slotIndex} member of that element <b>must</b> be currently associated with a frame picture <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">matching</a> the video picture resource specified by the {@code pPictureResource} member of the same element at the time the command is executed on the device</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR and an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-active-reference-picture-info">active reference picture</a> corresponding to any element of {@code pDecodeInfo→pReferenceSlots} represents a top field, then the DPB slot index of the bound video session specified by the {@code slotIndex} member of that element <b>must</b> be currently associated with a top field picture <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">matching</a> the video picture resource specified by the {@code pPictureResource} member of the same element at the time the command is executed on the device</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR and an <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-h264-active-reference-picture-info">active reference picture</a> corresponding to any element of {@code pDecodeInfo→pReferenceSlots} represents a bottom field, then the DPB slot index of the bound video session specified by the {@code slotIndex} member of that element <b>must</b> be currently associated with a bottom field picture <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">matching</a> the video picture resource specified by the {@code pPictureResource} member of the same element at the time the command is executed on the device</li>
            ...
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR and ##VkVideoDecodeAV1ProfileInfoKHR{@code ::filmGrainSupport} set to #FALSE, then <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-av1-film-grain">film grain</a> <b>must</b> not be enabled for the decoded picture</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, and <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#decode-av1-film-grain">film grain</a> is enabled for the decoded picture, then the video picture resources specified by {@code pDecodeInfo→dstPictureResource} and {@code pDecodeInfo→pSetupReferenceSlot→pPictureResource} <b>must</b> not <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html\#video-picture-resource-matching">match</a></li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> include a ##VkVideoDecodeAV1PictureInfoKHR structure</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then the {@code frameHeaderOffset} member of the ##VkVideoDecodeAV1PictureInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> be less than the minimum of {@code pDecodeInfo→srcBufferRange}</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then all elements of the {@code pTileOffsets} member of the ##VkVideoDecodeAV1PictureInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> be less than {@code pDecodeInfo→srcBufferRange}</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then for each element <code>i</code> of the {@code pTileOffsets} and {@code pTileSizes} members of the ##VkVideoDecodeAV1PictureInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} the sum of {@code pTileOffsets}[i] and {@code pTileSizes}[i] <b>must</b> be less than or equal to {@code pDecodeInfo→srcBufferRange}</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR and {@code pDecodeInfo→pSetupReferenceSlot} is not {@code NULL}, then the {@code pNext} chain of {@code pDecodeInfo→pSetupReferenceSlot} <b>must</b> include a ##VkVideoDecodeAV1DpbSlotInfoKHR structure</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then the {@code pNext} chain of each element of {@code pDecodeInfo→pReferenceSlots} <b>must</b> include a ##VkVideoDecodeAV1DpbSlotInfoKHR structure</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then each element of the {@code referenceNameSlotIndices} array member of the ##VkVideoDecodeAV1PictureInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo} <b>must</b> either be negative or <b>must</b> equal the {@code slotIndex} member of one of the elements of {@code pDecodeInfo→pReferenceSlots}</li>
            <li>If the bound video session was created with the video codec operation #VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR, then the {@code slotIndex} member of each element of {@code pDecodeInfo→pReferenceSlots} <b>must</b> equal one of the elements of the {@code referenceNameSlotIndices} array member of the ##VkVideoDecodeAV1PictureInfoKHR structure included in the {@code pNext} chain of {@code pDecodeInfo}</li>
        </ul>
