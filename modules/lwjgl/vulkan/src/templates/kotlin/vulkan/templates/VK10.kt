...
val VK10 = "VK10".nativeClass(Module.VULKAN, "VK10", prefix = "VK", binding = VK_BINDING_INSTANCE) {
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS_KHR A buffer creation or memory allocation failed because the requested address is not available.</li>
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#QUERY_TYPE_PERFORMANCE_QUERY_KHR specifies a <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#queries-performance">performance query</a>.</li>
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT_KHR specifies that the buffer&#8217;s address <b>can</b> be saved and reused on a subsequent run (e.g. for trace capture and replay), see ##VkBufferOpaqueCaptureAddressCreateInfoKHR for more detail.</li>
        </ul>
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR specifies that the buffer <b>can</b> be used to retrieve a buffer device address via #GetBufferDeviceAddressKHR() and use that address to access the buffer&#8217;s memory from a shader.</li>
        </ul>
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#IMAGE_LAYOUT_DEPTH_ATTACHMENT_STENCIL_READ_ONLY_OPTIMAL specifies a layout for depth/stencil format images allowing read and write access to the depth aspect as a depth attachment, and read only access to the stencil aspect as a stencil attachment or in shaders. It is equivalent to #IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL_KHR and #IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL_KHR.</li>
            <li>#IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL_KHR specifies a layout for the depth aspect of a depth/stencil format image allowing read and write access as a depth attachment.</li>
            <li>#IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL_KHR specifies a layout for the depth aspect of a depth/stencil format image allowing read-only access as a depth attachment or in shaders.</li>
            <li>#IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL_KHR specifies a layout for the stencil aspect of a depth/stencil format image allowing read and write access as a stencil attachment.</li>
            <li>#IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL_KHR specifies a layout for the stencil aspect of a depth/stencil format image allowing read-only access as a stencil attachment or in shaders.</li>
        ...
        <h5>See Also</h5>
        ##VkAttachmentDescription, ##VkAttachmentDescription2KHR, ##VkAttachmentDescriptionStencilLayoutKHR, ##VkAttachmentReference, ##VkAttachmentReference2KHR, ##VkAttachmentReferenceStencilLayoutKHR, ##VkDescriptorImageInfo, ##VkImageCreateInfo, ##VkImageMemoryBarrier, #CmdBindShadingRateImageNV(), #CmdBlitImage(), #CmdClearColorImage(), #CmdClearDepthStencilImage(), #CmdCopyBufferToImage(), #CmdCopyImage(), #CmdCopyImageToBuffer(), #CmdResolveImage()
        """,
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#PRIMITIVE_TOPOLOGY_LINE_LIST_WITH_ADJACENCY specifies a series of <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#drawing-line-lists-with-adjacency">separate line primitives with adjacency</a>.</li>
            <li>#PRIMITIVE_TOPOLOGY_LINE_STRIP_WITH_ADJACENCY specifies a series of <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#drawing-line-strips-with-adjacency">connected line primitives with adjacency</a>, with consecutive primitives sharing three vertices.</li>
            <li>#PRIMITIVE_TOPOLOGY_TRIANGLE_LIST_WITH_ADJACENCY specifies a series of <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#drawing-triangle-lists-with-adjacency">separate triangle primitives with adjacency</a>.</li>
            <li>#PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP_WITH_ADJACENCY specifies <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#drawing-triangle-strips-with-adjacency">connected triangle primitives with adjacency</a>, with consecutive triangles sharing an edge.</li>
...
￿    VkFence                                     fence);</code></pre>
        ...
        <ul>
            ...
            <li>All elements of the {@code pWaitSemaphores} member of all elements of {@code pSubmits} created with a {@code VkSemaphoreTypeKHR} of #SEMAPHORE_TYPE_BINARY_KHR <b>must</b> reference a semaphore signal operation that has been submitted for execution and any semaphore signal operations on which it depends (if any) <b>must</b> have also been submitted for execution</li>
            ...
            <li>If any element of {@code pSubmits}-&gt;pCommandBuffers includes a <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-queue-transfers-acquire">Queue Family Transfer Acquire Operation</a>, there <b>must</b> exist a previously submitted <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-queue-transfers-release">Queue Family Transfer Release Operation</a> on a queue in the queue family identified by the acquire operation, with parameters matching the acquire operation as defined in the definition of such <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-queue-transfers-acquire">acquire operations</a>, and which happens before the acquire operation</li>
            <li>If a command recorded into any element of {@code pCommandBuffers} was a #CmdBeginQuery() whose {@code queryPool} was created with a {@code queryType} of #QUERY_TYPE_PERFORMANCE_QUERY_KHR, the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#profiling-lock">profiling lock</a> <b>must</b> have been held continuously on the {@code VkDevice} that {@code queue} was retrieved from, throughout recording of those command buffers</li>
...
￿    VkFence                                     fence);</code></pre>
        ...
        <ul>
            ...
            <li>All elements of the {@code pWaitSemaphores} member of all elements of {@code pBindInfo} created with a {@code VkSemaphoreTypeKHR} of #SEMAPHORE_TYPE_BINARY_KHR <b>must</b> reference a semaphore signal operation that has been submitted for execution and any semaphore signal operations on which it depends (if any) <b>must</b> have also been submitted for execution.</li>
        </ul>
...
￿    VkSubresourceLayout*                        pLayout);</code></pre>
        ...
        <h5>Description</h5>
        If the image is <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#glossary-linear-resource">linear</a>, then the returned layout is valid for <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#memory-device-hostaccess">host access</a>.
...
￿    VkFramebuffer*                              pFramebuffer);</code></pre>
        ...
        <h5>Valid Usage</h5>
        <ul>
            <li>If {@code pCreateInfo}-&gt;flags does not include #FRAMEBUFFER_CREATE_IMAGELESS_BIT_KHR, and {@code attachmentCount} is not 0, each element of {@code pCreateInfo}-&gt;pAttachments <b>must</b> have been created on {@code device}</li>
        </ul>
...
￿    const float                                 blendConstants[4]);</code></pre>
        ...
        VkCommandBuffer("commandBuffer", "the command buffer into which the command will be recorded."),
        Check(4)..float.const.p("blendConstants", "a pointer to an array of four values specifying the R, G, B, and A components of the blend constant color used in blending, depending on the <a target=\"_blank\" href=\"https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\\#framebuffer-blendfactors\">blend factor</a>.")
    )
...
￿    const VkImageSubresourceRange*              pRanges);</code></pre>
        ...
        VkClearColorValue.const.p("pColor", "a pointer to a ##VkClearColorValue structure containing the values that the image subresource ranges will be cleared to (see <a target=\"_blank\" href=\"https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\\#clears-values\">Clear Values</a> below)."),
...
￿    const VkImageSubresourceRange*              pRanges);</code></pre>
        ...
        VkClearDepthStencilValue.const.p("pDepthStencil", "a pointer to a ##VkClearDepthStencilValue structure containing the values that the depth and stencil image subresource ranges will be cleared to (see <a target=\"_blank\" href=\"https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\\#clears-values\">Clear Values</a> below)."),
        AutoSize("pRanges")..uint32_t("rangeCount", "the number of image subresource range structures in {@code pRanges}."),
        VkImageSubresourceRange.const.p("pRanges", "a pointer to an array of ##VkImageSubresourceRange structures describing a range of mipmap levels, array layers, and aspects to be cleared, as described in <a target=\"_blank\" href=\"https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\\#resources-image-views\">Image Views</a>.")
    )
...
￿    const VkImageMemoryBarrier*                 pImageMemoryBarriers);</code></pre>
        ...
        Unlike #CmdPipelineBarrier(), a <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-queue-transfers">queue family ownership transfer</a> <b>cannot</b> be performed using #CmdWaitEvents().
        </div>
        ...
        <ul>
            ...
            <li>The {@code srcAccessMask} member of each element of {@code pMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code srcStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code dstAccessMask} member of each element of {@code pMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code dstStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code srcAccessMask} member of each element of {@code pBufferMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code srcStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code dstAccessMask} member of each element of {@code pBufferMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code dstStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code srcAccessMask} member of each element of {@code pImageMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code srcStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code dstAccessMask} member of any element of {@code pImageMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code dstStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
        </ul>
...
￿    const VkImageMemoryBarrier*                 pImageMemoryBarriers);</code></pre>
        ...
        <ul>
            ...
            <li>The {@code srcAccessMask} member of each element of {@code pMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code srcStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>The {@code dstAccessMask} member of each element of {@code pMemoryBarriers} <b>must</b> only include access flags that are supported by one or more of the pipeline stages in {@code dstStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>For any element of {@code pBufferMemoryBarriers}, if its {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} members are equal, or if its {@code srcQueueFamilyIndex} is the queue family index that was used to create the command pool that {@code commandBuffer} was allocated from, then its {@code srcAccessMask} member <b>must</b> only contain access flags that are supported by one or more of the pipeline stages in {@code srcStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
            <li>For any element of {@code pBufferMemoryBarriers}, if its {@code srcQueueFamilyIndex} and {@code dstQueueFamilyIndex} members are equal, or if its {@code dstQueueFamilyIndex} is the queue family index that was used to create the command pool that {@code commandBuffer} was allocated from, then its {@code dstAccessMask} member <b>must</b> only contain access flags that are supported by one or more of the pipeline stages in {@code dstStageMask}, as specified in the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.1-extensions/html/vkspec.html\#synchronization-access-types-supported">table of supported access types</a></li>
...
￿    uint32_t                                    query);</code></pre>
        ...
        <ul>
            ...
            <li>If {@code queryPool} was created with a {@code queryType} of #QUERY_TYPE_PERFORMANCE_QUERY_KHR and one or more of the counters used to create {@code queryPool} was #QUERY_SCOPE_COMMAND_BUFFER_KHR, the #CmdEndQuery() <b>must</b> be the last recorded command in {@code commandBuffer}</li>
            <li>If {@code queryPool} was created with a {@code queryType} of #QUERY_TYPE_PERFORMANCE_QUERY_KHR and one or more of the counters used to create {@code queryPool} was #QUERY_SCOPE_RENDER_PASS_KHR, the #CmdEndQuery() <b>must</b> not be recorded within a render pass instance</li>
        </ul>
