...
val VK10 = "VK10".nativeClass(Module.VULKAN, "VK10", prefix = "VK", binding = VK_BINDING_INSTANCE) {
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_NV specifies the execution of vkCmdBuildAccelerationStructureNV, vkCmdCopyAccelerationStructureNV, and vkCmdWriteAccelerationStructuresPropertiesNV.</li>
            <li>
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#INDEX_TYPE_NONE_NV specifies no indices are provided.</li>
        </ul>
...
￿    VkIndexType                                 indexType);</code></pre>
        ...
        <ul>
            ...
            <li>{@code indexType} <b>must</b> not be VK_INDEX_TYPE_NONE_NV.</li>
        </ul>
...
￿    VkFilter                                    filter);</code></pre>
        ...
        <ul>
            ...
            <li>If {@code filter} is #FILTER_CUBIC_IMG, then the <a target="_blank" href="https://www.khronos.org/registry/vulkan/specs/1.0-extensions/html/vkspec.html\#resources-image-format-features">format features</a> of {@code srcImage} <b>must</b> contain #FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_IMG.</li>
            <li>If {@code filter} is #FILTER_CUBIC_IMG, {@code srcImage} <b>must</b> have a {@code VkImageType} of #IMAGE_TYPE_2D</li>
            <li>If {@code commandBuffer} is an unprotected command buffer, then {@code srcImage} <b>must</b> not be a protected image</li>
            <li>If {@code commandBuffer} is an unprotected command buffer, then {@code dstImage} <b>must</b> not be a protected image</li>
            <li>If {@code commandBuffer} is a protected command buffer, then {@code dstImage} <b>must</b> not be an unprotected image</li>
            <li>The {@code srcSubresource.mipLevel} member of each element of {@code pRegions} <b>must</b> be less than the {@code mipLevels} specified in ##VkImageCreateInfo when {@code srcImage} was created</li>
