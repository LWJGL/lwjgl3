 ...
 * 
 * <p>As with {@code VkPipelineCache}, the binary cache depends on the hardware architecture. Therefore the application must assume the cache might fail, and thus need to handle falling back to the original PTX code as necessary. Most often, the cache will succeed if the same GPU driver and architecture is used between the cache generation from PTX and the use of this cache. But most often, in the event of a new driver version or a if using a different GPU But in the event of a new driver version or if using a different GPU architecture, the cache is likely to become invalid.</p>
 * 
...
public class NVCudaKernelLaunch {
     ...
     * 
     * <ul>
     * <li>{@link #VK_DEBUG_REPORT_OBJECT_TYPE_CUDA_MODULE_NV DEBUG_REPORT_OBJECT_TYPE_CUDA_MODULE_NV}</li>
     * <li>{@link #VK_DEBUG_REPORT_OBJECT_TYPE_CUDA_FUNCTION_NV DEBUG_REPORT_OBJECT_TYPE_CUDA_FUNCTION_NV}</li>
     * </ul>
     */
    public static final int
     ...
     * <h5>Description</h5>
     * 
     * <p>Once a CUDA module has been created, you <b>may</b> create the function entry point that <b>must</b> refer to one function in the module.</p>
     * 
