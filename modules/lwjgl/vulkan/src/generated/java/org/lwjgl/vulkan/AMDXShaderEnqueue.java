...
public class AMDXShaderEnqueue {
     ...
     * <h5>Description</h5>
     * 
     * <p>This command <b>must</b> be called before using {@code scratch} to dispatch the bound execution graph pipeline.</p>
     * 
     ...
     * <li>{@code scratchSize} <b>must</b> be greater than or equal to {@link VkExecutionGraphPipelineScratchSizeAMDX}{@code ::minSize} returned by {@link #vkGetExecutionGraphPipelineScratchSizeAMDX GetExecutionGraphPipelineScratchSizeAMDX} for the bound execution graph pipeline</li>
     ...
     * <p>Execution of this command <b>may</b> modify any memory locations in the range [{@code scratch},{@code scratch} + {@code scratchSize}). Accesses to this memory range are performed in the {@link VK13#VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT PIPELINE_STAGE_2_COMPUTE_SHADER_BIT} pipeline stage with the {@link VK13#VK_ACCESS_2_SHADER_STORAGE_READ_BIT ACCESS_2_SHADER_STORAGE_READ_BIT} and {@link VK13#VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT ACCESS_2_SHADER_STORAGE_WRITE_BIT} access flags.</p>
     * 
     * <p>This command <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#executiongraphs-meshnodes-statecapture">captures command buffer state</a> for mesh nodes similarly to draw commands.</p>
     * 
     * <h5>Valid Usage</h5>
     * 
     * <ul>
     ...
     * <li>The device memory range [{@code scratch},{@code scratch} {@code scratchSize}] <b>must</b> have been initialized with {@link #vkCmdInitializeGraphScratchMemoryAMDX CmdInitializeGraphScratchMemoryAMDX} using the bound execution graph pipeline, and not modified after that by anything other than another execution graph dispatch command</li>
     ...
     * <li>For each {@link VkDispatchGraphInfoAMDX} structure in {@code pCountInfo→infos}, {@code nodeIndex} <b>must</b> be a valid node index in the bound execution graph pipeline, as returned by {@link #vkGetExecutionGraphPipelineNodeIndexAMDX GetExecutionGraphPipelineNodeIndexAMDX}</li>
     ...
     * <li>For each {@link VkDispatchGraphInfoAMDX} structure in {@code pCountInfo→infos}, {@code payloads} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
     * <li>For each {@link VkDispatchGraphInfoAMDX} structure in {@code pCountInfo→infos}, {@code nodeIndex} <b>must</b> be a valid node index in the bound execution graph pipeline, as returned by {@link #vkGetExecutionGraphPipelineNodeIndexAMDX GetExecutionGraphPipelineNodeIndexAMDX} when this command is executed on the device</li>
     ...
     * <li>For each {@link VkDispatchGraphInfoAMDX} structure in {@code countInfo→infos}, {@code payloads} <b>must</b> be a multiple of <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#limits-executionGraphDispatchAddressAlignment">{@code executionGraphDispatchAddressAlignment}</a></li>
