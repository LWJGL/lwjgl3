 ...
 * <dt><b>Contact</b></dt>
 * <dd><ul>
 * <li>Piers Daniell <a target="_blank" href="https://github.com/KhronosGroup/Vulkan-Docs/issues/new?title=VK_EXT_extended_dynamic_state:%20&amp;body=@pdaniell-nv%20">pdaniell-nv</a></li>
 * </ul></dd>
 * </dl>
 * 
...
public class EXTExtendedDynamicState {
     ...
     * <h5>C Specification</h5>
     * 
     * <p>Alternatively, to bind vertex buffers, along with their sizes and strides, to a command buffer for use in subsequent draw commands, call:</p>
     * 
     * <pre><code>
     ...
     * 
     * <p>If the bound pipeline state object was created with the {@link #VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE_EXT DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE_EXT} dynamic state enabled then {@code pStrides}[i] specifies the distance in bytes between two consecutive elements within the corresponding buffer. In this case the {@link VkVertexInputBindingDescription}{@code ::stride} state from the pipeline state object is ignored.</p>
     * 
     * <h5>Valid Usage</h5>
     * 
     * <ul>
     ...
     * <li>If the bound pipeline state object was created with the {@link #VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE_EXT DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE_EXT} dynamic state enabled then {@code pStrides} <b>must</b> not be {@code NULL}, otherwise {@code pStrides} <b>must</b> be {@code NULL}</li>
     ...
     * <li>If {@code pStrides} is not {@code NULL} each element of {@code pStrides} <b>must</b> be greater than or equal to the maximum extent of of all vertex input attributes fetched from the corresponding binding, where the extent is calculated as the VkVertexInputAttributeDescription::offset plus VkVertexInputAttributeDescription::format size</li>
     * </ul>
     * 
     * <h5>Valid Usage (Implicit)</h5>
     * 
     ...
     * @param pSizes        an optional array of the size in bytes of vertex data bound from {@code pBuffers}.
     * @param pStrides      optional, and when not {@code NULL} is a pointer to an array of buffer strides.
     */
     ...
     *
     * @param commandBuffer the command buffer into which the command will be recorded.
     * @param faceMask      a bitmask of VkStencilFaceFlagBits specifying the set of stencil state for which to update the stencil operation.
