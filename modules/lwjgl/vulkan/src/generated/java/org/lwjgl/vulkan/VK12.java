...
public class VK12 extends VK10 {
     ...
     * <h5>Description</h5>
     * 
     * <p>If the condition is satisfied when {@code vkWaitSemaphores} is called, then {@code vkWaitSemaphores} returns immediately. If the condition is not satisfied at the time {@code vkWaitSemaphores} is called, then {@code vkWaitSemaphores} will block and wait up to {@code timeout} nanoseconds for the condition to become satisfied.</p>
     * 
     * <p>If {@code timeout} is zero, then {@code vkWaitSemaphores} does not wait, but simply returns information about the current state of the semaphore. {@link VK10#VK_TIMEOUT TIMEOUT} will be returned in this case if the condition is not satisfied, even though no actual wait was performed.</p>
     * 
     * <p>If the specified timeout period expires before the condition is satisfied, {@code vkWaitSemaphores} returns {@link VK10#VK_TIMEOUT TIMEOUT}. If the condition is satisfied before {@code timeout} nanoseconds has expired, {@code vkWaitSemaphores} returns {@link VK10#VK_SUCCESS SUCCESS}.</p>
     * 
