...
public class EXTAcquireDrmDisplay {
     ...
     * <h5>Description</h5>
     * 
     * <p>All permissions necessary to control the display are granted to the Vulkan instance associated with the provided {@code physicalDevice} until the display is either released or the connector is unplugged. The provided {@code drmFd} <b>must</b> correspond to the one owned by the {@code physicalDevice}. If not, the error code {@link VK10#VK_ERROR_UNKNOWN ERROR_UNKNOWN} <b>must</b> be returned. The DRM FD must have DRM master permissions. If any error is encountered during the acquisition of the display, the call <b>must</b> return the error code {@link VK10#VK_ERROR_INITIALIZATION_FAILED ERROR_INITIALIZATION_FAILED}.</p>
     * 
     ...
     * <h5>Description</h5>
     * 
     * <p>If there is no {@code VkDisplayKHR} corresponding to the {@code connectorId} on the {@code physicalDevice}, the returning {@code display} <b>must</b> be set to {@link VK10#VK_NULL_HANDLE NULL_HANDLE}. The provided {@code drmFd} <b>must</b> correspond to the one owned by the {@code physicalDevice}. If not, the error code {@link VK10#VK_ERROR_UNKNOWN ERROR_UNKNOWN} <b>must</b> be returned. Master permissions are not required, because the file descriptor is just used for information gathering purposes. The given {@code connectorId} <b>must</b> be a resource owned by the provided {@code drmFd}. If not, the error code {@link VK10#VK_ERROR_UNKNOWN ERROR_UNKNOWN} <b>must</b> be returned. If any error is encountered during the identification of the display, the call <b>must</b> return the error code {@link VK10#VK_ERROR_INITIALIZATION_FAILED ERROR_INITIALIZATION_FAILED}.</p>
     * 
     * <h5>Valid Usage (Implicit)</h5>
     * 
     * <ul>
