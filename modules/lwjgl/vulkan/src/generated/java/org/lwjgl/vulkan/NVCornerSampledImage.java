 ...
 * 
 * <ul>
 * <li>Texels are centered on integer coordinates. See <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#textures-unnormalized-to-integer">Unnormalized Texel Coordinate Operations</a></li>
 * <li>Normalized coordinates are scaled using <code>coord × (dim - 1)</code> rather than <code>coord × dim</code>, where dim is the size of one dimension of the image. See <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#textures-normalized-to-unnormalized">normalized texel coordinate transform</a>.</li>
 * <li>Partial derivatives are scaled using <code>coord × (dim - 1)</code> rather than <code>coord × dim</code>. See <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#textures-scale-factor">Scale Factor Operation</a>.</li>
 * <li>Calculation of the next higher LOD size goes according to <code>⌈dim / 2⌉</code> rather than <code>⌊dim / 2⌋</code>. See <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#resources-image-mip-level-sizing">Image Mip Level Sizing</a>.</li>
 * <li>The minimum level size is 2x2 for 2D images and 2x2x2 for 3D images. See <a href="https://registry.khronos.org/vulkan/specs/latest/html/vkspec.html#resources-image-mip-level-sizing">Image Mip Level Sizing</a>.</li>
 * </ul>
 * 
 * <p>This image organization is designed to facilitate a system like Ptex with separate textures for each face of a subdivision or polygon mesh. Placing sample locations at pixel corners allows applications to maintain continuity between adjacent patches by duplicating values along shared edges. Additionally, using the modified mipmapping logic along with texture dimensions of the form <code>2<sup>n</sup>+1</code> allows continuity across shared edges even if the adjacent patches use different LOD values.</p>
 * 
 * <dl>
