...
nmeshopt_setAllocator(
    ...
    size_t(
        ...
        {@code destination} must contain enough space for the resulting remap table ({@code vertex_count} elements). {@code indices} can be #NULL if the input
        is unindexed.
        """,

        unsigned_int.p("destination", ""),
        Check("index_count")..nullable..unsigned_int.const.p("indices", ""), // assert(indices || index_count == vertex_count);
        size_t("index_count", ""),
    ...
    size_t(
        ...
        {@code meshopt_remapVertexBuffer} for each vertex stream.
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting vertex buffer ({@code unique_vertex_count} elements, returned by
        ...
        Check("vertex_count * vertex_size")..void.const.p("vertices", ""),
        UseVariable..AutoSize("remap")..size_t("vertex_count", ""),
        size_t("vertex_size", ""),
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting index buffer ({@code index_count} elements). {@code indices} can be #NULL if the input
        is unindexed.
        """,
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting index buffer ({@code index_count} elements)
        """,
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting index buffer ({@code index_count} elements). {@code cache_size} should be less than the
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting index buffer ({@code index_count} elements). {@code indices} must contain index data
        that is the result of #optimizeVertexCache() (<b>not</b> the original mesh indices!). {@code vertex_positions} should have {@code float3} position in
    ...
    size_t(
        ...
        {@code destination} must contain enough space for the resulting vertex buffer ({@code vertex_count} elements). {@code indices} is used both as an input
    ...
    size_t(
        ...
        {@code destination} must contain enough space for the resulting remap table ({@code vertex_count} elements)
        """,
    ...
    size_t(
        ...
        {@code buffer} must contain enough space for the encoded index buffer (use #encodeIndexBufferBound() to compute worst case size).
        """,
    ...
    size_t(
        ...
        streams, call {@code meshopt_encodeVertexBuffer} for each stream.

        {@code buffer} must contain enough space for the encoded vertex buffer (use #encodeVertexBufferBound() to compute worst case size).
        """,
    ...
    int(
        ...
        {@code destination} must contain enough space for the resulting vertex buffer ({@code vertex_count * vertex_size} bytes).
        """,
    ...
    size_t(
        ...
        {@code destination} must contain enough space for the target index buffer, worst case can be computed with #stripifyBound(). {@code restart_index}
        should be {@code 0xffff} or {@code 0xffffffff} depending on index size, or 0 to use degenerate triangles.
        """,
    ...
    size_t(
        ...
        {@code destination} must contain enough space for the target index buffer, worst case can be computed with #unstripifyBound().
        """,
    ...
    meshopt_OverdrawStatistics(
        ...
        {@code vertex_positions} should have {@code float3} position in the first 12 bytes of each vertex - similar to {@code glVertexPointer}.
        """,
    ...
    size_t(
        ...
        The resulting data can be used to render meshes using NVidia programmable mesh shading pipeline, or in other cluster-based renderers. For maximum
        efficiency the index buffer being converted has to be optimized for vertex cache first.

        {@code destination} must contain enough space for all meshlets, worst case size can be computed with #buildMeshletsBound(). {@code max_vertices} and
        {@code max_triangles} can't exceed limits statically declared in ##MeshoptMeshlet ({@code max_vertices} &le; 64, {@code max_triangles} &le; 126).
        """,

        Unsafe..meshopt_Meshlet.p("destination", ""),
        unsigned_int.const.p("indices", ""),
        AutoSize("indices")..size_t("index_count", ""),
        size_t("vertex_count", ""),
        size_t("max_vertices", ""),
    ...
    meshopt_Bounds(
        ...
        {@code vertex_positions} should have {@code float3} position in the first 12 bytes of each vertex - similar to {@code glVertexPointer}.
        {@code index_count} should be less than or equal to {@code 256*3} (the function assumes clusters of limited size).
        """,
    ...
    void(
        ...
        {@code destination} must contain enough space for the resulting index buffer ({@code index_count} elements). {@code vertex_positions} should have
        {@code float3} position in the first 12 bytes of each vertex - similar to {@code glVertexPointer}.
        """,
