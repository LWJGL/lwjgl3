...
val XR10 = "XR10".nativeClass(Module.OPENXR, "XR10", prefix = "XR", binding = XR_BINDING_INSTANCE) {
    ...
    EnumConstant(
        ...
        <table class="lwjgl">
            ...
            <tbody>
                ...
                <tr><td>#ERROR_SESSION_RUNNING</td><td>The session <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">is already running</a>.</td></tr>
                <tr><td>#ERROR_SESSION_NOT_RUNNING</td><td>The session <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_not_running">is not yet running</a>.</td></tr>
    ...
    EnumConstant(
        ...
        <h5>Description</h5>
        Most structures containing {@code type} members have a value of {@code type} matching the type of the structure, as described more fully in <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#valid-usage-for-structure-types">Valid Usage for Structure Types</a>.
    ...
    EnumConstant(
        ...
        <ul>
            ...
            <li>#SESSION_STATE_READY. The application is ready to call #BeginSession() and <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">sync its frame loop with the runtime.</a></li>
            ...
            <li>#SESSION_STATE_VISIBLE. The application has <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">synced its frame loop with the runtime</a> and is visible to the user but cannot receive XR input.</li>
            <li>#SESSION_STATE_FOCUSED. The application has <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">synced its frame loop with the runtime</a>, is visible to the user and can receive XR input.</li>
        ...
        Receiving the #SESSION_STATE_SYNCHRONIZED state indicates that the application has <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">synchronized its frame loop with the runtime</a>, but its frames are not visible to the user. The application <b>should</b> continue running its frame loop by calling #WaitFrame(), #BeginFrame() and #EndFrame(), although it should avoid heavy GPU work so that other visible applications can take CPU and GPU precedence. The application can save resources here by skipping rendering and not submitting any composition layers until #WaitFrame() returns an ##XrFrameState with {@code shouldRender} set to true. A runtime <b>may</b> use this frame synchronization to facilitate seamless switching from a previous XR application to this application on a frame boundary.

        Receiving the #SESSION_STATE_VISIBLE state indicates that the application has <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">synchronized its frame loop with the runtime</a>, and the session’s frames will be visible to the user, but the session is not eligible to receive XR input. An application may be visible but not have focus, for example when the runtime is composing a modal pop-up on top of the application’s rendered frames. The application <b>should</b> continue running its frame loop, rendering and submitting its composition layers, although it may wish to pause its experience, as users cannot interact with the application at this time. It is important for applications to continue rendering when visible, even when they do not have focus, so the user continues to see something reasonable underneath modal pop-ups. Runtimes <b>should</b> make input actions inactive while the application is unfocused, and applications should react to an inactive input action by skipping rendering of that action’s input avatar (depictions of hands or other tracked objects controlled by the user).

        Receiving the #SESSION_STATE_FOCUSED state indicates that the application has <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#sync_frame_loop">synchronized its frame loop with the runtime</a>, the session’s frames will be visible to the user, and the session is eligible to receive XR input. The runtime <b>should</b> only give one session XR input focus at any given time. The application <b>should</b> be running its frame loop, rendering and submitting composition layers, including input avatars (depictions of hands or other tracked objects controlled by the user) for any input actions that are active. The runtime <b>should</b> avoid rendering its own input avatars when an application is focused, unless input from a given source is being captured by the runtime at the moment.
...
￿    XrSystemId*                                 systemId);</code></pre>
        ...
        <h5>Description</h5>
        To get an {@code XrSystemId}, an application specifies its desired <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#form_factor_description">form factor</a> to #GetSystem() and gets the runtime’s {@code XrSystemId} associated with that configuration.
...
￿    const XrSessionBeginInfo*                   beginInfo);</code></pre>
        ...
        After this function successfully returns, the session <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">is considered to be running</a>. The application <b>should</b> then start its frame loop consisting of some sequence of #WaitFrame()/#BeginFrame()/#EndFrame() calls.

        If the session <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">is already running</a> when the application calls #BeginSession(), the runtime <b>must</b> return error #ERROR_SESSION_RUNNING. If the session <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_not_running">is not running</a> when the application calls #BeginSession(), but the session is not yet in the #SESSION_STATE_READY state, the runtime <b>must</b> return error #ERROR_SESSION_NOT_READY.
        ...
        Runtime session frame state <b>must</b> start in a reset state when a session transitions to <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">running</a> so that no state is carried over from when the same session was previously running. Frame state in this context includes #WaitFrame(), #BeginFrame(), and #EndFrame() call order enforcement.
...
￿    XrSession                                   session);</code></pre>
        ...
        <h5>Description</h5>
        When the application receives ##XrEventDataSessionStateChanged event with the #SESSION_STATE_STOPPING state, the application should stop its frame loop and then call #EndSession() to end the <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">running</a> session. This function signals to the runtime that the application will no longer call #WaitFrame(), #BeginFrame() or #EndFrame() from any thread allowing the runtime to safely transition the session to #SESSION_STATE_IDLE. The application <b>must</b> also avoid reading input state or sending haptic output after calling #EndSession().
        ...
        XrSession("session", "a handle to a <a href=\"https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\\#session_running\">running</a> {@code XrSession}.")
    )
    ...
    XrResult(
        ...
        <h5>C Specification</h5>
        When an application wishes to exit a <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">running</a> session, it <b>can</b> call #RequestExitSession(), requesting that the runtime transition through the various intermediate session states including #SESSION_STATE_STOPPING to #SESSION_STATE_EXITING.
...
￿    XrSession                                   session);</code></pre>
        ...
        <h5>Description</h5>
        If {@code session} <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_not_running">is not running</a> when #RequestExitSession() is called, #ERROR_SESSION_NOT_RUNNING <b>must</b> be returned.
...
￿    XrFrameState*                               frameState);</code></pre>
        ...
        The runtime <b>must</b> return #ERROR_SESSION_NOT_RUNNING if the {@code session} <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_not_running">is not running</a>.
...
￿    const XrFrameBeginInfo*                     frameBeginInfo);</code></pre>
        ...
        The runtime <b>must</b> return the error code #ERROR_CALL_ORDER_INVALID if there was no corresponding successful call to #WaitFrame(). The runtime <b>must</b> return the success code #FRAME_DISCARDED if a prior #BeginFrame() has been called without an intervening call to #EndFrame(). Refer to #BeginSession() for details on how a transition to <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_running">session running</a> resets the frame function call order.
...
￿    const XrFrameEndInfo*                       frameEndInfo);</code></pre>
        ...
        #ERROR_SESSION_NOT_RUNNING <b>must</b> be returned if the {@code session} <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#session_not_running">is not running</a>.
...
￿    XrActionSet                                 actionSet);</code></pre>
        ...
        The implementation <b>must</b> not free underlying resources for the action set while there are other valid handles that refer to those resources. The implementation <b>may</b> release resources for an action set when all of the action spaces for actions in that action set have been destroyed. See <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#spaces-action-spaces-lifetime">Action Spaces Lifetime</a> for details.
...
￿    const XrInteractionProfileSuggestedBinding* suggestedBindings);</code></pre>
        ...
        If the interaction profile path does not follow the structure defined in <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#semantic-path-interaction-profiles">Interaction Profiles</a> or suggested bindings contain paths that do not follow the format defined in <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#semantic-path-input">Device input subpaths</a>, the runtime <b>must</b> return #ERROR_PATH_UNSUPPORTED. If the interaction profile or input source for any of the suggested bindings does not exist in the allowlist defined in <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#semantic-path-interaction-profiles">Interaction Profile Paths</a>, the runtime <b>must</b> return #ERROR_PATH_UNSUPPORTED. A runtime <b>must</b> accept every valid binding in the allowlist though it is free to ignore any of them.
...
￿    XrInteractionProfileState*                  interactionProfile);</code></pre>
        ...
        If #AttachSessionActionSets() has not yet been called for the {@code session}, the runtime <b>must</b> return #ERROR_ACTIONSET_NOT_ATTACHED. If {@code topLevelUserPath} is not one of the device input subpaths described in section <a href="https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html\#semantic-path-user">/user paths</a>, the runtime <b>must</b> return #ERROR_PATH_UNSUPPORTED.
