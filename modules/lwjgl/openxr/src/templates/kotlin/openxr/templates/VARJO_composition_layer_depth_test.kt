...
val VARJO_composition_layer_depth_test = "VARJOCompositionLayerDepthTest".nativeClassXR("VARJO_composition_layer_depth_test", type = "instance", postfix = "VARJO") {
    documentation =
        ...
        Core OpenXR specifies that layer compositing must happen in the layer submission order (as described in <a href="https://registry.khronos.org/OpenXR/specs/1.1/html/xrspec.html\#rendering-compositing">rendering-compositing</a>). However, an application may want to composite the final image against the other layers based on depth information for proper occlusion. Layers can now provide depth information that will be used to calculate occlusion between those layers, as well as with the environment depth estimator ({@link VARJOEnvironmentDepthEstimation XR_VARJO_environment_depth_estimation}) when enabled.
