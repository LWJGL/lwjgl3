 ...
 * render to fast memory when it is not large enough to contain the actual frame buffer.</p>
 * 
 * <p>One approach to solving this problem is to design the GPU and/or driver using a tiled rendering architecture. With this approach the render target is
 ...
 * using this method is usually more efficient than rendering directly to slow memory.</p>
 * 
 * <p>This extension allows the application to specify a rectangular tile rendering area and have full control over the resolves for that area. The
 ...
 * GPU operations.</p>
 * 
 * <p>One typical use case could involve an application only rendering to select portions of the render target using this technique (which shall be referred
 ...
 * of the untouched portions of the frame buffer will be undefined).</p>
 * 
 * <p>Additionally, care must be taken to avoid the cost of mixing rendering with and without application tiling within a single frame. Rendering without
