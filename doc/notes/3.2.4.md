...
- GLFW: Added overrides for internally loaded shared libraries.
    ...
    * `GLFWVulkan.setPath`
- GL/GLES/AL: Added `createCapabilities` overloads that accept a `PointerBuffer`. Enables explicit management of the off-heap memory used by the capabilities instances.
